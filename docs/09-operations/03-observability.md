...
> **Not production-ready.** GKE Prometheus + Alertmanager deployed on 2026-06-18 with ClusterIP-only Services; real Slack/SMTP receivers validated via direct transport and GKE Alertmanager retest (`GKEAlertPipelineValidationNoChannel` succeeded, POST 200, notifications email/slack 1, failures 0, 3 targets active). Sustained 30min load alert firing + A-03/A-04 re-signoff remain open; FIND-001 is deployed+receivers validated, not resolved. Templates in `infrastructure/production/kubernetes/configmaps/alertmanager-config.yaml` and `infrastructure/production/alertmanager/alertmanager-prod.yml` are placeholder-only in repo (real ConfigMap generated from Secret out-of-band before apply).
