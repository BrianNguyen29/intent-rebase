...
| FIND-005 | LOW | Security | Penetration test not executed; scope defined only as internal planning artifact. No external pen test team engaged. **Engagement packet prepared** (`docs/08-security/08-external-pentest-engagement.md`) for vendor handoff. | 🟡 SIGNED AS APPROVED WITH CONDITIONS — self-scan prep (ZAP 0 FAIL, 1 WARN accepted) completed; engagement packet prepared; no external tester engaged. Accepted as non-blocking for production transition pending external pentest scheduling. | Engage external pen test team; execute against staging environment; remediate any HIGH/CRITICAL findings with evidence. |
...
| Area | Status | Notes |
|------|--------|-------|
| SRE Operational Readiness | 🟡 SELF-ATTESTED-SOLO — EXTERNAL RE-SIGNOFF NOT OBTAINED | Historical 2026-06-15 `APPROVED WITH CONDITIONS` by DuongNguyen remains on record. Post-2026-06-15 changes reviewed by project owner (self-attestation) for private-only operation. FIND-001 (sustained-load alert firing) and FIND-004 (RPO/RTO not measured) remain open. ESO sync + GKE observability + 30-min load evidence addresses initial concerns but does not close the gate. For production/public claims, external re-signoff is required. See `docs/13-adrs/16-solo-private-operation-waiver.md`. |
| Security Architecture | 🟡 SELF-ATTESTED-SOLO — EXTERNAL RE-SIGNOFF NOT OBTAINED | Historical 2026-06-15 `APPROVED WITH CONDITIONS` by DuongNguyen remains on record. Post-2026-06-15 changes reviewed by project owner (self-attestation) for private-only operation. FIND-002 (RLS partial) and FIND-003 (broader secret rotation program not completed) remain open. GSM provisioned + ESO sync + staging rotation + prod API key rotation validated addresses initial FIND-003 concern but does not close the gate. ZAP self-scan is prep only; does not close A-07. For production/public claims, external re-signoff is required. See `docs/13-adrs/16-solo-private-operation-waiver.md`. |
| Pen Test Results (A-07) | 🔴 WAIVED-SOLO / PRIVATE-ONLY — EXTERNAL NOT APPROVED | ZAP self-scan prep completed (0 FAIL, 1 WARN accepted); scope defined; isolated staging env ready; engagement packet prepared (`docs/08-security/08-external-pentest-engagement.md`). External pen test is **waived for private-only operation** because this is a personal solo project with no budget for third-party testing. A-07 remains **NOT APPROVED** for any production/public claim. **No self-scan or assistant-generated note substitutes for an external pen test.** See `docs/13-adrs/16-solo-private-operation-waiver.md`. |
| Overall Recommendation | 🟡 SELF-ATTESTED-SOLO — EXTERNAL RE-SIGNOFF NOT OBTAINED | Initial conditions (FIND-001 through FIND-005) have substantive progress evidence but remain open. A-03/A-04 are self-attested by project owner for private-only operation; external re-signoff has not been obtained. A-07 is waived for private-only operation; remains NOT APPROVED for production/public claims. Remaining items are tracked as work items with documented owner and timeline. For production/public claims, all external gates require named third-party evidence. See `docs/13-adrs/16-solo-private-operation-waiver.md`. |
...
| G-EXT-4 | Staging / production load testing (L3–L5) | 30-MIN STAGING + 20 VU + 50 VU PASSED — NOT PROD PUBLIC INGRESS LOAD | Backend Lead / SRE | L3: staged k6/Artillery results; L4: 30min sustained load + 5 VU/20 VU/50 VU real app SLO rule validation + all alert types + real receivers; L5: production load test results — all remain open before production claim |
| G-OPS-1 | Backup/restore executed and validated against production-like infrastructure | 🟡 VALIDATED — CLONE-ONLY + EMPIRICAL RPO ~2.7s (2026-06-22) | Backend Lead | Automated restore test pass log; backup integrity verification (checksum + sample restore); RPO empirically measured (~2.7s WAL lag via live marker + PITR clone verification); formal RTO with live-traffic cutover and scheduled DR drills remain open. Cloud SQL clone provisioning observed at ~20 minutes; RPO/RTO not committed as SLA. |
...
|-------------|----------|----------|
| Lane 4: Real SLO Rules | `IntentApiLatencyP95High` and `IntentApiErrorRateHigh` applied to Prometheus; validated under bounded 5 VU, 20 VU, and 50 VU k6 load; temporary `AppMetricsValidationRule` fired and removed | `docs/09-operations/11-slo-targets.md` §5.6–5.8 |
...
| Lane 2: NATS app consumer | `NATS_URL=nats://nats:4222`, `NATS_TOKEN` token auth enforced, `INTENT_API_NATS_CONSUMER=true`, `INTENT_API_NATS_FULL_CONSUMER=true`, `INTENT_API_NATS_DLQ_WORKER=true`, `INTENT_API_NATS_DLQ_REPLAY_WORKER=true` wired; `audit_events` stream created; three consumers (checkpoint_creator, snapshot_creator, notifier) polling; DLQ metrics/replay workers started | `infrastructure/production/README.md` §NATS App Consumer Wired |
| Lane 3: Forensic bucket | `gs://forensic-evidence-ferrum-497801-ed2c5bdd` created with versioning, 30-day retention (UNLOCKED), PAP enforced, uniform access; **GCS BundleStorage backend implemented 2026-06-22** using metadata-server OAuth (no HMAC keys); end-to-end smoke test passed (upload/download/delete verified, 30-day retention active); app wired with `FORENSIC_BUNDLE_STORAGE=gcs` and `FORENSIC_BUNDLE_BUCKET=forensic-evidence-ferrum-497801-ed2c5bdd`; not S3 Object Lock compliant | `infrastructure/production/README.md` §Dedicated Forensic Immutable Bucket / §Forensic Bundle Runtime Wiring |
...
| Lane 5: RPO measurement | **Empirical RPO ~2.7 seconds measured** via live marker→WAL archive lag + PITR clone verification (2026-06-22); Cloud SQL backup config: PITR enabled, WAL archiving enabled, transaction log retention 7 days; theoretical RPO < 1 min; single measurement, not statistically significant, not committed SLA | `docs/09-operations/07-backup-restore.md` §RPO Measurement |
| Lane 4: 20 VU / 50 VU load tests | 20 VU: 7219 iterations, p95=760µs, 0% 5xx, ~68.7 req/s; 50 VU: 17989 iterations, p95=127µs, 0% 5xx, ~171 req/s; Prometheus SLO rules remained `inactive` throughout; single-replica deployment on 2-node GKE cluster with 50m CPU request; no saturation observed | `docs/09-operations/11-slo-targets.md` §5.7–5.8 |
...
- Phase B (lanes 7–11): Completed 2026-06-21/22
...
- No empirical RPO committed as SLA (single measurement ~2.7s, not statistically significant)
- No formal live-traffic DR cutover
- No external pen test vendor engaged
- No SLO breach firing validated (artificial latency/error injection not tested)
...
| 2026-06-22 | BrianNguyen (via authorized assistant fixer) | **Evidence refresh:** 20 VU and 50 VU k6 load test results added to §2026-06-22 Evidence Refresh; empirical RPO ~2.7s measured and added; SLO rules validated at 5/20/50 VU (breach firing still open); G-EXT-4 and G-OPS-1 status updated in Appendix A; Phase B status corrected (lane 9 completed, lane 11 completed). No external reviewer engaged. No sign-off claimed. A-07 remains NOT APPROVED. A-03/A-04 remain PENDING DIRECT REVIEWER CONFIRMATION. No production-ready claim. `git diff --check` pass. |
