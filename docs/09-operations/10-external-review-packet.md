...
| Observability Checklist | LOCAL DOCKER-COMPOSE (bounded) + RECEIVERS VALIDATED (2026-06-18) | `docs/09-operations/09-observability-evidence-checklist.md` | Metrics endpoint, Prometheus scrape, Grafana provisioning, one alert firing validated locally; real Slack/SMTP credentials validated via direct transport send and temporary Alertmanager POST (2026-06-18). GKE Alertmanager/Prometheus Deployment is NOT deployed; production telemetry not connected. |
...
| G-OPS-3 | Observability stack deployed with production telemetry and real receivers | LOCAL DOCKER-COMPOSE (bounded) + RECEIVERS VALIDATED (2026-06-18) — GKE STACK OPEN | Backend Lead | Production Prometheus/Grafana/Alertmanager deployment on GKE; real receiver validation under sustained load; all alert types fire with validated routing |
...
| 2026-06-18 | BrianNguyen (via authorized assistant fixer) | Alert validation completed: real Slack/SMTP credentials supplied out-of-band to K8s Secrets `app-secrets` in both `intent-rebase` and `intent-rebase-staging` namespaces (18 keys each, including both uppercase and lowercase Slack/SMTP keys). Direct transport validation: Slack webhook returned `200 ok`; SMTP send returned `sent` with `0` refused. Temporary Alertmanager container validation: health OK, POST to `/api/v2/alerts` returned `200`, alert `SlackSMTPValidationTest` state `active`, receiver `slack-and-email`, logs showed no relevant errors. Temp files cleaned up. FIND-001 updated from OPEN to 🟡 PARTIALLY VALIDATED — RECEIVERS OK / GKE STACK OPEN. G-OPS-3 Observability Checklist updated to LOCAL DOCKER-COMPOSE (bounded) + RECEIVERS VALIDATED. GKE Alertmanager/Prometheus Deployment remains NOT deployed; production telemetry stack remains open. A-07 remains NOT APPROVED. No real secrets committed in repo. No production-ready claim. `infrastructure/production/README.md` Forbidden Claims and completion items updated. `infrastructure/production/kubernetes/configmaps/alertmanager-config.yaml` header updated to clarify template placeholders and GKE stack not deployed. `infrastructure/production/kubernetes/secrets/app-secrets.example.yaml` and `.env.example` gained `smtp_to`/`SMTP_TO` placeholders.
