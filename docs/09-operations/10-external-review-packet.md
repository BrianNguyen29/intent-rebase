...
| Security Architecture | 🟡 PENDING DIRECT REVIEWER CONFIRMATION | FIND-002 (RLS partial) and FIND-003 (prod rotation not done) remain open. GSM provisioned + ESO sync + staging rotation validated addresses initial FIND-003 concern but does not close the gate. ZAP self-scan is prep only; does not close A-07. |
...
| G-EXT-2 | External security architecture review (authn/authz, RLS, threat model, residual risks) | APPROVED WITH CONDITIONS (2026-06-15) — NOT PRODUCTION-READY | DuongNguyen | FIND-002, FIND-003 prod rotation not done, FIND-005 remain open; Section H signed 2026-06-15. Any 2026-06-19 notes are assistant-generated and require direct reviewer confirmation. |
...
| G-OPS-2 | Secrets rotation validated in production environment | 🟡 STAGING ROTATION VALIDATED — PROD ROTATION NOT DONE | Backend Lead | Live rotation execution log; secret audit log; Vault/AWS SM integration verified — ESO/GSM sync validated for staging; prod rotation was NOT performed. |
