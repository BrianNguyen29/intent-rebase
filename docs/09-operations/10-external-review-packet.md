...
|------------|----------|----------|-------------|--------|------------|
| FIND-001 | MED | SRE | Production telemetry not connected; Alertmanager real receivers (PagerDuty/Slack/email) are missing. **Update 2026-06-18:** GCP core scaffold applied (VPC, GKE, GCS, Cloud SQL) and Kubernetes namespace `intent-rebase` created; Alertmanager ConfigMap and Secret templates passed server dry-run only. Slack/SMTP not configured. Alertmanager remains placeholder-only. | OPEN | Deploy production monitoring stack; configure real Alertmanager receivers; validate all alert types fire under sustained load. |
...
| FIND-003 | MED | Security | Secret rotation is template-only. **Update 2026-06-18:** Kubernetes Secrets template (`app-secrets.example.yaml`) passed server dry-run against the provisioned GKE cluster; no real secrets applied. No Vault/AWS Secrets Manager deployed. No live key rotation validated. | OPEN | Deploy secret manager (Vault/AWS SM); implement and validate API key + JWT signing key rotation with grace window. |
| FIND-004 | LOW | SRE | Backup/restore validated locally only (`pg_dump`/`pg_restore` against docker-compose Postgres). **Update 2026-06-18:** Cloud SQL Postgres provisioned with backups and PITR enabled (`point_in_time_recovery_enabled = true`). No restore or PITR test executed against the provisioned instance; RPO/RTO not measured. | OPEN | Execute and verify production backup/restore against production-like infrastructure; validate RPO=1h and RTO=30m. |
