...
| FIND-004 | LOW | SRE | Backup/restore validated locally only (`pg_dump`/`pg_restore` against docker-compose Postgres). **Update 2026-06-18:** Cloud SQL Postgres provisioned with backups and PITR enabled (`point_in_time_recovery_enabled = true`). Internal smoke deploy completed with app running against provisioned DB. No restore or PITR test executed against the provisioned instance; RPO/RTO not measured. | OPEN | Execute and verify production backup/restore against production-like infrastructure; validate RPO=1h and RTO=30m. |
