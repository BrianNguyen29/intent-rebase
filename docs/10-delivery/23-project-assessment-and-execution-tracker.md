...
|----|--------|------------|-------|--------------|
| **S1** | NATS tenant isolation design & implementation | Oracle recommendation; `docs/14-governance/08-tenant-isolation.md` | Backend Lead | None |
| **S2** | RLS enforcement audit tool (static check or test harness verifying all DML paths use `begin_with_tenant`) | Oracle recommendation | Backend Lead | None |
| **S3** | NATS publisher implementation (for trace propagation foundation) | Oracle recommendation | Backend Lead | None |
...
| **S5** | Add startup warning when `INTENT_API_REQUIRE_JWT=false` | Risk R-6 mitigation | Backend Lead | None |
