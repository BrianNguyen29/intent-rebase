...
| **S7** | 🟡 DESIGN / LOCAL RUNBOOK DONE + METRIC CODE-COMPLETE LOCALLY — Panic alerting integration design/runbook delivered (`docs/09-operations/12-panic-alerting-integration.md`); RB15 panic response runbook added to `05-runbooks.md`; Prometheus alert rule design-only; `process_panics_total` counter implemented locally via `record_panic_event()` inside `panic_hook()` (`crates/intent-api/src/panic_hardening.rs`) with thread-local test-recorder unit test (`test_record_panic_event_increments_counter`). Production alerting remains blocked: alert rule not deployed, no external receivers, no staging env, no SRE sign-off. | `22-phase-4-entry-plan.md` A-08 | Backend Lead | None |
...
| 2026-06-07 | BrianNguyen (via authorized assistant fixer) | Phase 2 S7 metric code-complete locally — added `process_panics_total` counter increment via new `pub(crate) fn record_panic_event()` called inside `panic_hook()` in `crates/intent-api/src/panic_hardening.rs`. Added bounded unit test `test_record_panic_event_increments_counter` in `panic_hardening_tests.rs` that uses `metrics::with_local_recorder` with a minimal custom `Recorder` impl backed by `metrics::atomics::AtomicU64` to capture the `process_panics_total` increment without touching the global `metrics-exporter-prometheus` recorder (avoids global metrics-recorder conflicts). Updated `docs/09-operations/12-panic-alerting-integration.md` (§2 current state, §3 alert path, §3.1 prerequisite note, §4.1 test count to 10, §4.2 expected evaluation, §4.4 load-test observability, §6 blocked prerequisites metric row, §7 forbidden-claim wording, and §9 update log). Updated `docs/09-operations/05-runbooks.md` RB15 (status, symptoms, caveats) to reflect metric implemented locally while preserving production alerting blockers. Tracker Phase 2 table S7 row updated: 🟡 DESIGN / LOCAL RUNBOOK DONE + METRIC CODE-COMPLETE LOCALLY. Verification: `cargo fmt --all -- --check` pass, `cargo test -p intent-api --lib panic_hardening` 10/10 pass, `cargo check -p intent-api --all-features` pass, `cargo clippy -p intent-api --all-features -- -D warnings` pass, `git diff --check` pass. No production-readiness claim; alert rule still design-only; staging/receivers/SRE sign-off remain blocked. |
| 2026-06-07 | BrianNguyen (via authorized assistant fixer) | P1 Webhook SQL Repository Wiring slice delivered (local bounded wiring). In `crates/intent-api/src/main.rs`, added `webhook_subscription_repo::SqlxWebhookSubscriptionRepository` to the `intent_api::{...}` import block. In both `build_sql_router_with_consumer_jwt` (JWT SQL router path) and `build_sql_router_with_consumer_impl` (non-JWT SQL router path), replaced the two `None` placeholders for `webhook_subscription_repo` and `webhook_outbox_repo` with `Some(Arc::new(SqlxWebhookSubscriptionRepository::new(pool.clone())))` and `Some(Arc::new(SqlxWebhookOutboxRepository::new(pool.clone())))` respectively, and removed the stale "not wired in main.rs yet" comments. The in-memory router (`build_inmemory_router`) and the `maybe_start_webhook_outbox_worker` path are untouched (worker remains default-off behind `INTENT_API_WEBHOOK_OUTBOX_WORKER`). New internal evidence doc `docs/10-delivery/24c-webhook-sql-repo-wiring-evidence.md` records the wiring change, the in-memory / SQL non-JWT / SQL JWT wiring table, sequential verification gates, and explicit local-dev / default-off-worker caveats. `docs/10-delivery/24-strategic-roadmap-and-checklist.md` §3 status row updated to `🟡 BOUNDED WIRING DONE (SQL router startup now passes concrete SqlxWebhookSubscriptionRepository + SqlxWebhookOutboxRepository)`, §6.5 action checklist 4 of 4 items checked off (item 3 — the actual SQL router startup wiring — newly delivered by this slice; items 1, 2, 4 confirmed via pre-existing Slice 4a/4b + WEB-LOCAL-1 evidence and the canonical ignored integration test path), and new §6.10 evidence-link subsection added. Verification: `cargo fmt --all -- --check` pass, `cargo check -p intent-api --all-features` pass, `cargo clippy -p intent-api --all-features -- -D warnings` pass, `cargo test -p intent-api --lib` 442/442 + 17 ignored pass, `git diff --check` pass, and optional ignored `DATABASE_URL=postgres://intent_rebase:intent_rebase_dev@localhost:5432/intent_rebase_phase1_fix cargo test -p intent-api --test webhook_integration -- --ignored` 1/1 pass (local docker-compose Postgres; `intent_rebase_phase1_fix` is the Phase 1 fix DB with migrations applied — not the default `intent_rebase` DB which lacks the migrations). P0 uncommitted changes (orchestrator test, roadmap/tracker/evidence docs) preserved untouched. No production-readiness claim; local-dev bounded evidence only. External gates (A-03/A-04/A-05/A-06/A-07/A-10/A-12/A-13) remain blocked; A-11 remains deferred/SDK-blocked. |
| 2026-06-07 | BrianNguyen (via authorized assistant fixer) | Strategic roadmap and checklist added — created `docs/10-delivery/24-strategic-roadmap-and-checklist.md` capturing the latest internal strategic evaluation recommendations in full detail: P0 runtime adapter end-to-end proof, P1 intent-api decomposition, P1 webhook SQL repository wiring, P2 intent-cli decoupling, P2 benchmark compile guard, P2 documentation language policy, P3/P4 deferred/external-gated. Each item has problem statement, why now, scope IN/OUT, action checklist, acceptance criteria, validation commands, risks/tradeoffs, and owner type (local / user decision / external). Includes a "Do not do yet" anti-recommendations section (no production IaC/K8s/Terraform, no CI-green badge, no new ops docs for nonexistent infra, no more feature sprawl before P0, no public exposure of internal roadmap, no self-signed external evidence) and a local verification policy mapping fmt/check/clippy/targeted tests/git diff --check/public-doc leakage scan. This document is **internal planning only** — it is not a public support doc, not production-readiness evidence, and not a CI-green claim. External gates (A-03/A-04/A-05/A-06/A-07/A-10/A-12/A-13) remain blocked; A-11 remains deferred/SDK-blocked. Brief pointer added to `docs/10-delivery/20-project-completion-roadmap.md` Related Documents (no content duplication). Public docs (`README.md`, `README.vi.md`, `docs/README.md`, `docs/getting-started/`, `docs/reference/`, `.github/`, `CONTRIBUTING`, `SECURITY`) untouched. Verification: `git diff --check` pass. No code changes. |
| 2026-06-07 | BrianNguyen (via authorized assistant fixer) | P0 Runtime Adapter End-to-End Proof slice delivered (local bounded proof). Added non-ignored test `test_runtime_adapter_apply_end_to_end` at the end of `crates/rebase-orchestrator/src/orchestrator_tests.rs`. Test drives a single apply cycle through the runtime adapter boundary using `MockAdapter::ready()`: (1) `RebaseOrchestrator::align_checkpoint` resolves the planner's `phase1_baseline` selection to the seeded real checkpoint (asserts `checkpoint_id == Some(seeded_id)`, `outcome == CheckpointAlignmentOutcome::ClosestMatch`, non-empty `rationale`); (2) `RebaseOrchestrator::send_runtime_rebase_signal` runs `is_adapter_ready` → `send_rebase_signal` → `replay_from_checkpoint` (asserts `signal_sent`, `status == Succeeded`, `replay_attempted`, `replay_completed`, status message contains `"Signal sent and replay completed"`); (3) `is_runtime_ready()` + `MockAdapter::is_adapter_ready()` round-trip confirms the call path uses the same `Arc<dyn RuntimeAdapter>` seam. 3 of 5 `RuntimeAdapter` methods exercised in sequence. Verification: `cargo fmt --all -- --check` pass, `cargo test -p rebase-orchestrator --lib test_runtime_adapter_apply_end_to_end` 1/1 pass, full `cargo test -p rebase-orchestrator --lib` 41/41 pass (40 pre-existing + 1 new), `cargo check -p rebase-orchestrator --all-features` pass, `cargo clippy -p rebase-orchestrator --all-features -- -D warnings` pass, `git diff --check` pass. New internal evidence doc `docs/10-delivery/24b-runtime-adapter-e2e-evidence.md` records the test, command, result, trait-method coverage table, sequential verification gates, and explicit non-production caveat. `docs/10-delivery/24-strategic-roadmap-and-checklist.md` §3 status row flipped to `🟡 BOUNDED DONE (local proof)`, §4.5 action checklist 5 of 7 items checked off (2 deferred with rationale: `intent-api` filtered test out of scope; `docs/getting-started/configuration.md` cross-link forbidden by the slice handoff), new §4.10 evidence link section added, §14 update log row added. No public-doc edits. No production-readiness claim. External gates (A-03/A-04/A-05/A-06/A-07/A-10/A-12/A-13) remain blocked; A-11 remains deferred/SDK-blocked. |
| 2026-06-07 | BrianNguyen (via authorized assistant fixer) | P2 Intent CLI Decoupling slice delivered (binary-to-library split, local bounded). Created `crates/intent-cli/src/lib.rs` exposing `pub struct Cli`, `pub enum Commands`, `pub fn run(cli: Cli) -> anyhow::Result<()>`, `pub fn init_tracing()` (idempotent `try_init`), and `pub fn build_run_payload(action_ids: &[Uuid], intent_id: Option<Uuid>, initiated_by: Option<&str>) -> serde_json::Value` (pure helper extracted from `run_orchestration`). The `Run` / `GetRun` subcommands and the `#[arg]` / `#[command]` attributes are byte-identical at the source level, so user-visible CLI behavior is preserved. Rewrote `crates/intent-cli/src/main.rs` as an 11-line thin entry point: `fn main() -> anyhow::Result<()> { intent_cli::run(intent_cli::Cli::parse()) }`. `crates/intent-cli/Cargo.toml` gained explicit `[lib] name = "intent_cli" path = "src/lib.rs"` and `[[bin]] name = "intent-cli" path = "src/main.rs"`; `[dependencies]` and `[dev-dependencies]` unchanged. Three new unit tests in `lib.rs` (`build_run_payload_includes_all_fields`, `build_run_payload_emits_nulls_for_optional_fields`, `build_run_payload_has_exactly_three_top_level_keys`) verify the JSON request-body shape end-to-end without a live HTTP server. Verification: `cargo fmt --all -- --check` pass, `cargo check -p intent-cli --all-targets` pass, `cargo clippy -p intent-cli --all-targets -- -D warnings` pass, `cargo test -p intent-cli --lib` 3/3 pass, `git diff --check` pass. The `cargo run -p intent-cli -- --help` step is recorded as **behavior-preserved** (same panic as pre-refactor) — a pre-existing latent duplicate-short-flag bug for `-a` on `api_url` and `api_key` exists in the original `main.rs` (verified by stashing the P2 diff and re-running `--help` against the un-refactored binary, which panics with the identical message); the bug is documented in the new evidence doc and routed to a follow-up slice per the "no change to CLI flags" guardrail. `docs/10-delivery/24-strategic-roadmap-and-checklist.md` §3 status row flipped from `⬜ Not started` to `🟡 BOUNDED DONE (binary-to-library split with 3 new unit tests; see §7.10)`, §7.5 action checklist 5 of 5 items checked off, new §7.10 evidence-link subsection added, new update-log row at §14. New internal evidence doc `docs/10-delivery/24d-intent-cli-decoupling-evidence.md` records the refactor, the new test coverage table, sequential verification gates, the pre-existing `--help` panic, and explicit local-bounded caveats. Public docs (`README.md`, `README.vi.md`, `docs/README.md`, `docs/getting-started/`, `docs/reference/`, `.github/`, `CONTRIBUTING`, `SECURITY`) untouched. No production-readiness claim; external gates (A-03..A-13) remain blocked / deferred. |
| 2026-06-07 | BrianNguyen (via authorized assistant fixer) | Pre-existing `intent-cli --help` panic resolved within the P2 scope. In `crates/intent-cli/src/lib.rs`, added `short = 'u'` to the top-level `api_url` field and `short = 'k'` to the top-level `api_key` field so the two fields no longer collide on the default short form (`-a`). Long flags (`--api-url`, `--tenant-id`, `--api-key`), the default `http://localhost:8080`, the `Run` / `GetRun` subcommand shapes, and all HTTP payload semantics are unchanged; the bounded scope is one `#[arg]` attribute correction per conflicting top-level flag. After the fix, `cargo run -p intent-cli -- --help` exits 0 and prints the clap-generated help (top-level options `-u` / `--api-url`, `-t` / `--tenant-id`, `-k` / `--api-key`, `-h` / `--help`; subcommands `run` and `get-run`). No test changes were required (existing 3 `build_run_payload` tests still pass). `docs/10-delivery/24d-intent-cli-decoupling-evidence.md` §4.1 verification table flipped the `Help output` row to **Pass**, §5 reframed from "Pre-Existing `--help` Behavior (Not Introduced by This Slice)" to "`--help` Panic Fix (Duplicate Short Flag)" with the exact one-line-per-flag diff, the new help text, and the explicit out-of-scope note for any latent Run-subcommand short collisions, and §2.1 / §6 caveats updated. `docs/10-delivery/24-strategic-roadmap-and-checklist.md` §7.6 acceptance criterion reworded to reflect "help succeeds" instead of "produces the same help text as before"; §7.10 evidence-link paragraph and bullet updated to drop the "behavior-preserved" wording and to point at the now-resolved short-flag correction; §14 update log gained a new row. Sequential verification re-run: `cargo fmt --all -- --check` pass, `cargo check -p intent-cli --all-targets` pass, `cargo clippy -p intent-cli --all-targets -- -D warnings` pass, `cargo test -p intent-cli --lib` 3/3 pass, `cargo run -p intent-cli -- --help` **pass**, `git diff --check` pass. No public-doc edits. No production-readiness claim. External gates (A-03..A-13) remain blocked / deferred. |
| 2026-06-07 | BrianNguyen (via authorized assistant fixer) | P1 Intent API Decomposition — first bounded demo slice delivered (`health_routes` → `routes::health`). Moved all five items from `crates/intent-api/src/health_routes.rs` (`request_id_middleware`, `trace_context_middleware`, `health_handler`, `ready_handler`, `metrics_handler`) into `crates/intent-api/src/routes/health.rs` and made the module self-contained; `add_routes` now references them as local symbols. `crates/intent-api/src/lib.rs` lost `pub mod health_routes;` and the stale "moved to health_routes.rs" comment. `crates/intent-api/src/router.rs` now references `routes::health::request_id_middleware` and `routes::health::trace_context_middleware` (middleware layering order preserved). The now-orphan `crates/intent-api/src/health_routes.rs` was deleted. `docs/04-api/route-openapi-contract-map.md` (internal) `Handler Module` column updated for `/health`, `/ready`, `/metrics`. Route paths, methods, status codes, headers, request/response bodies, middleware ordering, and metrics behavior are byte-identical to the pre-extraction implementation. New internal evidence doc `docs/10-delivery/24e-health-routes-extraction-evidence.md` records the move, the reference-rewrite table, the deletion, sequential verification gates, and the explicit non-production caveat. `docs/10-delivery/24-strategic-roadmap-and-checklist.md` §3 status row updated, §5.5 action checklist 5 of 6 items checked off (the `22-phase-4-entry-plan.md` A-09 cross-update is deferred to a follow-up A-09 continuation slice per the slice handoff), new §5.10 evidence-link subsection added, §8 P2 Benchmark Compile Guard stale wording corrected (real criterion source files exist in all 4 crates; remaining bounded work is compile-guard verification, not creating missing files), and §14 update log gained two rows. Sequential verification: `cargo fmt --all -- --check` pass, `cargo check -p intent-api --all-features` pass, `cargo clippy -p intent-api --all-features -- -D warnings` pass, `cargo test -p intent-api --lib` 442/442 + 17 ignored pass, `cargo check --workspace --all-features` pass, `git diff --check` pass. No public-doc edits. No production-readiness claim. External gates (A-03..A-13) remain blocked / deferred. A-11 remains deferred/SDK-blocked. |
| 2026-06-07 | BrianNguyen (via authorized assistant fixer) | P2 Benchmark Compile Guard — bounded local slice delivered (compile-only, no timings). `cargo fmt --all -- --check` pass (exit 0, < 1s). `cargo bench --workspace --no-run` pass (exit 0, 16m 58s first build, all 7 `benches/<name>.rs` source files compiled into optimized bench-profile executables under `target/release/deps/`). `git diff --check` pass (exit 0, working tree clean). Compiled bench files: `http_handlers` (intent-api), `graph_ops` + `graph_traversal` (graph-service), `db_operations` + `query_latency` (intent-service), `diff_latency` + `rebase_latency` (rebase-engine). Informational observation (no code change): `intent-service/benches/query_latency.rs` and `rebase-engine/benches/diff_latency.rs` are **not** declared in any `[[bench]]` stanza in their respective `Cargo.toml` files, but they compile and link as bench executables because Cargo's bench auto-discovery picks them up. New internal evidence doc `docs/10-delivery/24f-benchmark-compile-guard-evidence.md` records the commands, results, compiled-executable table, per-bench fixture/feature/env table (pure/in-memory vs `DATABASE_URL`-gated self-skipping vs loopback-requiring), known limitations (compile-only, no `--all-features` exercise, no clippy-on-benches re-run, no `cargo test --workspace --lib --all-features` re-run since the prior slices already green on this HEAD), and explicit local-bounded caveats. `docs/10-delivery/24-strategic-roadmap-and-checklist.md` §3 status row flipped from `⬜ Not started` to `🟡 BOUNDED DONE (local compile guard)`, §8.5 action checklist 4 of 4 items checked off (the previous slice's two deferred items — `cargo bench --workspace --no-run` evidence run and the per-bench fixture/feature doc — are now closed), new §8.10 evidence-link subsection added, §13 relationship table now references 24b/24c/24d/24e/24f evidence docs, and §14 update log gained this row. No public-doc edits. No production-readiness claim. External gates (A-03..A-13) remain blocked / deferred. A-11 remains deferred/SDK-blocked. |
| 2026-06-07 | BrianNguyen (via authorized assistant fixer) | P2 Documentation Language Policy — user decision accepted (option A: English-primary technical docs; Vietnamese allowed only in `README.vi.md` or explicit `*.vi.md` translation files). User accepted the policy on 2026-06-07. New internal evidence doc `docs/10-delivery/24g-documentation-language-policy-evidence.md` records the user decision, the policy text verbatim, the future-bounded-slice migration checklist (survey → triage → translate / pair / label → verify → record), the acceptance criteria a future migration slice must meet (bounded, public-doc leakage scan clean, affirmative-claim scan clean, no new `.vi.md` outside allowlist, evidence doc with classification per file, no public-doc edits, no enforcement tooling, non-production caveat), the verification commands (Vietnamese-content scan, affirmative-claim scan, `find -name "*.vi.md"`, `git diff --check`), and a non-production boilerplate for future migration evidence docs. `docs/10-delivery/24-strategic-roadmap-and-checklist.md` §3 status row flipped from `⬜ Not started` to `✅ ACCEPTED (user decision)`; §2 executive-summary row annotated with the acceptance; §9.5 rewritten from "Recommended Policy (proposed, pending user decision)" to "Accepted Policy (user decision on 2026-06-07)"; §9.6 action checklist split into 9.6.a (policy-acceptance items, all checked off) and 9.6.b (future bounded slices, all planned, not executed); §9.7 acceptance criteria added; §9.8 validation commands added; §9.11 evidence-link subsection added; §13 relationship table now references the new 24g evidence doc; §14 update log gained a new row. **No doc was translated, renamed, or deleted by this slice; no public doc was touched; no code change.** The actual migration is deferred to future bounded slices per the policy's "Future migrations must be bounded, reviewed, and verify no public/internal leakage or overclaim" guardrail. Public docs (`README.md`, `README.vi.md`, `docs/README.md`, `docs/getting-started/`, `docs/reference/`, `.github/`, `CONTRIBUTING`, `SECURITY`) untouched. No production-readiness claim. External gates (A-03..A-13) remain blocked / deferred. A-11 remains deferred/SDK-blocked. Verification: `git diff --check` pass. No `cargo` commands were run by this slice (it is a docs-only acceptance slice with no code change). |
| 2026-06-08 | BrianNguyen (via authorized assistant fixer) | P2 Documentation Language Policy — survey slice delivered. New internal evidence doc `docs/10-delivery/24h-documentation-language-survey-evidence.md` records the workspace-wide scan (`rg -c` for Vietnamese diacritics, 159 `*.md` files, 70 with hits), public-doc verdict (PASS), classification inventory, priority tiers, and next-slice recommendations. `docs/10-delivery/24-strategic-roadmap-and-checklist.md` §9.6.b survey item checked off; new §9.12 survey evidence link added; §14 update log gained this row. **No doc was translated, renamed, or deleted; no public doc was touched; no code change.** Verification: `git diff --check` pass. No `cargo` commands run. Non-production caveat preserved. External gates (A-03..A-13) remain blocked / deferred. A-11 remains deferred/SDK-blocked. |
| 2026-06-08 | BrianNguyen (via authorized assistant fixer) | P2 Documentation Language Policy — Migration slice 1 partial delivery. `docs/13-adrs/01-runtime-adapter.md` translated from Vietnamese/mixed original-authored prose to English in place (~22 lines changed across Context, Decision, Rationale, and Consequences). Classification: `translate` (full). Verification: Vietnamese-diacritic scan (clean), public-doc leakage scan (clean), affirmative-claim scan (clean), no-new-`.vi.md` check (only `README.vi.md`), `git diff --check` (pass). New internal evidence doc `docs/10-delivery/24i-runtime-adapter-adr-language-migration-evidence.md` records the classification, changed file, segments translated, verification results, and non-production caveat. `docs/10-delivery/24-strategic-roadmap-and-checklist.md` §9.6.b migration slice 1 item updated to partial-complete wording; new §9.13 evidence-link subsection added; §14 update log gained this row. `docs/10-delivery/04-phase-2-runtime-integrated.md` and `docs/10-delivery/01-roadmap.md` remain pending for future bounded slices. No public docs touched; no `.vi.md` added; no code changes. External gates (A-03..A-13) remain blocked. A-11 remains deferred/SDK-blocked. |
...
| 2026-06-08 | BrianNguyen (via authorized assistant fixer) | Docs cleanup — removed five stale duplicate root-level delivery checklist stubs (`checklist-phase-0.md` through `checklist-phase-4.md`) after fixing the sole internal link that referenced a stub. Link in `docs/10-delivery/10-phase-2b-residual-risk-deferral-register.md` updated from `./checklist-phase-2.md` to `./checklists/checklist-phase-2.md`. Canonical checklists under `docs/10-delivery/checklists/` untouched. Verification: `git diff --check` pass; no remaining Markdown links to `./checklist-phase-*.md` from `docs/10-delivery/*.md` outside `checklists/`. No public docs touched; no code changes. |
| 2026-06-10 | BrianNguyen (via authorized assistant fixer) | Remaining todo list created — `docs/10-delivery/25-remaining-todo-list.md` encodes the verified audit gaps into a canonical internal todo-list separating completed work, local-executable remaining tasks (Tier 3 translations, ADR/governance READMEs, mixed delivery docs, code TODOs, stale contradiction cleanup, intent-cli short flag audit, benchmark manifest symmetry), risky/design-first work, and external-gated blockers. Tracker §13 gains this compact update-log row. `24-strategic-roadmap-and-checklist.md` §14 gains a companion row. No public docs touched. No code changes. No production-readiness claim. |
| 2026-06-10 | BrianNguyen (via authorized assistant fixer) | Slice B (ADR/governance README translations + minor single-word fix) executed. Translated `docs/13-adrs/README.md` (~10 lines: headers, intro prose, table header, contributing list). Translated `docs/14-governance/README.md` (~15 lines: headers, intro prose, table headers/cells, rules header). Fixed `docs/09-operations/03-observability.md` L28 (`Structured logs với:` → `Structured logs with:`). Vietnamese-diacritic scan clean on all three target files. `git diff --check` pass. No public docs touched. No new `.vi.md`. No code changes. No production-readiness claim. External gates remain blocked. |
| 2026-06-10 | BrianNguyen (via authorized assistant fixer) | Slice C (mixed delivery docs — 10 files) executed. Translated Vietnamese fragments in `docs/10-delivery/02-phase-0-foundations.md` (7 lines), `03-phase-1-core-control-plane.md` (3 lines), `06-phase-4-expansion.md` (3 lines), `11-phase-2b-sign-off-packet.md` (1 line), `checklists/README.md` (12 lines), `checklists/checklist-phase-0.md` (3 lines), `checklists/checklist-phase-1.md` (3 lines), `checklists/checklist-phase-2.md` (3 lines), `checklists/checklist-phase-3.md` (3 lines), `checklists/checklist-phase-4.md` (2 lines). Preserved tables, checklists, status caveats, and task-status semantics. Vietnamese-diacritic scan clean on all ten target files. Public-doc leakage scan clean. Affirmative-claim scan clean. No new `.vi.md`. `git diff --check` pass. No public docs touched. No code changes. No production-readiness claim. External gates remain blocked. A-11 remains deferred/SDK-blocked. |
| 2026-06-10 | BrianNguyen (via authorized assistant fixer) | 25d low-risk remaining tasks evidence recorded — `docs/10-delivery/25d-low-risk-remaining-tasks-evidence.md` created. Consolidates D2 (`graph.rs` placeholder wording), G (benchmark manifest symmetry), F (CLI short-flag audit/fix), E (stale contradiction cleanup: A-09 continuation note, C-8 wording polish), and D1 deferred rationale. Tracker §13 gains this row. `24-strategic-roadmap-and-checklist.md` §14 gains companion row. No public docs touched. No code changes. No production-readiness claim. External gates remain blocked. A-11 remains deferred/SDK-blocked. |
| 2026-06-10 | BrianNguyen (via authorized assistant fixer) | Tier 3 Batch 1 (`docs/01-product/`) translated. Five files (`01-product-thesis.md`, `02-goals-nongoals.md`, `03-core-principles.md`, `04-use-cases.md`, `05-glossary.md`) translated from Vietnamese to English in place. Remaining Tier 3 count: 29 files. Verification: Vietnamese-diacritic scan clean on all five target files; public-doc leakage scan clean; affirmative-claim scan clean; no new `.vi.md`; `git diff --check` pass. New evidence doc `docs/10-delivery/25e-documentation-language-tier3-product-evidence.md` created with internal solo sign-off. Tracker §13 and `24-strategic-roadmap-and-checklist.md` §14 gain update-log rows. No public docs touched. No code changes. No production-readiness claim. External gates remain blocked. A-11 remains deferred/SDK-blocked. |
