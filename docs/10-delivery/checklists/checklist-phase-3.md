...
**Exit Gate:** Phase 3 exit gate when all items are checked and have evidence.
...
**P4 Bounded Slice — Items 5-1 through 5-5, 5-8 (verification), 5-9 (export), 5-10 (download) Delivered; full replay (5-6/5-7), async orchestration, storage lifecycle remain Phase 4 scope**
...
[x] Bundle generation API: `POST /forensic/bundle` — P4 bounded slice (bounded synchronous path with real collection + in-memory storage; S3BundleStorage seam deferred to Phase 4)
    ...
    - Note: Bounded synchronous path: (1) ForensicDataCollector collects real data from intent/graph/audit repositories, (2) BundleGeneratorService generates manifest with integrity hashes, (3) storage backend selected via FORENSIC_BUNDLE_STORAGE env (s3 → S3BundleStorage, absent/default → InMemoryBundleStorage), (4) bundle status=Ready recorded in repository. NOT claimed: S3 Object Lock, async job orchestration, full runtime replay, chain-hash linkage, S3 lifecycle enforcement.
