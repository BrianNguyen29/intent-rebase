...
| T-07 | P1 | Artifact side-effect tx boundary | Backend Lead | ✅ BOUNDED IMPLEMENTED | ADR-08 in `docs/13-adrs/08-artifact-side-effect-tx-boundary.md` | Option A bounded implemented for SQL/RLS ingest path; non-RLS fallback preserved | GO: Bounded implemented per ADR-08; external SRE/security/load/pen and S3 Object Lock/full replay remain open |
...
| T-10 | P2 | rebase_apply handler review | Backend Lead | ✅ BOUNDED IMPLEMENTED — ADR-09 accepted; bounded D1–D7 delivered at commit `d98c7dc` | `docs/13-adrs/09-rebase-apply-rls-transaction-boundary.md`; `crates/intent-api/src/rebase_apply_handlers.rs`; `crates/intent-service/src/approval_request_repo.rs`; `crates/graph-service/src/sqlx_graph_repository.rs` | Bounded `BlockedManualReview` approval create/cancel RLS slice verified. Bounded D1–D7 delivered: D1 checkpoint read-only RLS tx; D2/D3/D4 graph update primary RLS tx via caller-side orchestration; D5 post-hoc helper removed; D6 RLC-14 RLS integration test added; D7 non-RLS fallback preserved and tested. Runtime signal remains post-commit/out-of-transaction by design. ADR-09 records accepted design with caller-side orchestration and non-RLS fallback. | GO: Bounded implemented per ADR-09; external SRE/security/load/pen and ADR-08/forensic RLS remain blockers |
