...
| T-10 | P2 | rebase_apply handler review | Backend Lead | 🟡 PARTIAL — bounded slice verified | `crates/intent-api/src/rebase_apply_handlers.rs`; `crates/intent-service/src/approval_request_repo.rs` | Bounded `BlockedManualReview` approval create/cancel RLS slice verified; full `AutoProceeded` graph/checkpoint/runtime wrapping remains blocked by missing tx-aware repository/orchestrator seams | GO: Full review remains open until graph/checkpoint tx design is resolved or explicitly deferred |
