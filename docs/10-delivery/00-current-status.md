...
| Runbooks | ✅ Bounded Delivered | RB6-RB10 delivered (rebase-stuck, approval-backlog, artifact-quarantine-fail, compensation-timeout, error-budget-burn) |
...
| Webhook delivery (B3-B14 bounded slice) | ✅ Bounded Delivered | Payload/header builders, async skeleton, env-gated dispatcher (`INTENT_API_WEBHOOK_DELIVERY`, default disabled), retry loop with incrementing `attempt_number` (B10), metrics counters (B11), RB13 runbook (B12), `WebhookDeliveryFailureRate` local alert rule (B13), webhook_subscriptions RLS test/helpers (B14); **no production delivery guarantees, no outbox, no HMAC/key rotation, no subscription CRUD API, no tokio::spawn fire-and-forget conversion** |
| Full apply env-gated dispatch integration test | 🔴 Missing | End-to-end integration test for webhook dispatch triggered by a rebase apply (via `INTENT_API_WEBHOOK_DELIVERY=true`) is not yet implemented; deferred — current coverage is unit/dispatcher-level only |
