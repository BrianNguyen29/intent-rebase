...
| JWT production guard + RLS helper scaffold | 🔄 In Progress | **Oracle-ordered P1 slices for full RLS transaction wrapping (P1-S1..S5 + RLC-4..RLC-9).** JWT tenant_id validation delivered (P3-S5 bounded slice); migration 009 consolidation with FORCE RLS delivered. **P1-S2 (IntentService.rls_pool wiring) LOCAL DONE (cargo fmt/check/test with/without jwt-auth passed, 221 tests); P1-S1 (RlsAwarePool shared), P1-S3 (RlsTransactionExt), P1-S4 (graph_edge wrap), P1-S5 (compensation/forensic/orchestration/approval/artifact wrap), RLC-4..RLC-9 (cross-tenant isolation tests) PENDING.** JWT guard (`INTENT_API_REQUIRE_JWT=true`) fails startup if JWT_SECRET missing/weak; `build_router_with_jwt_auth(...)` called from `main.rs`; `jwt_auth_async` middleware validates `tenant_id` claim (rejects nil UUID); `RlsTenantContext` struct with `set_rls_context`/`reset_rls_context`; RLS policies enabled via migration 009 consolidation; **full RLS enforcement pending completion of all P1 slices** |
