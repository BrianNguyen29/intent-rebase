...
**GCP scaffold + internal GKE smoke deploy (2026-06-18):** Core GCP infrastructure provisioned on project `ferrum-497801` (VPC, GKE, GCS, Cloud SQL Postgres). Internal smoke deploy completed with `intent-api:733e1ca` rolled out: pod `intent-api-6985547786-27hxd` running on node `gke-production-templ-production-templ-0abeaf6e-4g41` (IP `10.4.0.16`), SQL-backed health/ready endpoints responding, JWT guard passed, SQL-backed router initialized. PITR restore validated against separate Cloud SQL clone (2026-06-18): clone `pitr-restore-test-20260618084607` created at `2026-06-18T08:41:07Z`, operation `DONE` with no error, validation Job confirmed database and core tables present, clone deleted successfully. Clone-then-baseline sqlx metadata repair procedure validated against separate Cloud SQL clone (2026-06-18): clone `sqlx-baseline-test-20260618123303` created from source `production-template-postgres-ed2c5bdd`, clean DB `intent_rebase_clean` migrated with migration-mode Job, baseline imported (`COPY 22`, `sqlx_migrations_count=22`), verification against cloned existing DB succeeded (`relation "_sqlx_migrations" already exists, skipping`, `Migrations completed successfully`), final check confirmed `public_table_count=20`, `core_tables=_sqlx_migrations,graph_edges,graph_nodes,intent_versions,webhook_outbox`, `sqlx_migrations_count=22`, `failed_migrations=0`, clone deleted (`CLONE_DELETED`). Not production-ready: single-node cluster, no HPA, no ingress, no real Alertmanager receivers, no NATS, no S3, no secret manager, no load test, no pen test. **Primary/live DB `_sqlx_migrations` remains unrepaired**; clone-then-baseline validated on clone only. RPO/RTO not measured against live production traffic; full DR program maturity remains open.
