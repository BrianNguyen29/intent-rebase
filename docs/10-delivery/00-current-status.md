...
| **P2** | Webhook delivery production hardening | Outbox pattern (P2-6a schema design), background worker lifecycle (P2-6b design documented in [Production Readiness Backlog](./17-production-readiness-backlog.md)), HMAC signing, key rotation, subscription CRUD API, `tokio::spawn` fire-and-forget conversion | Backend Lead | 🔴 Deferred — Phase 4+ |
