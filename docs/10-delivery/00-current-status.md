...
| Full apply env-gated dispatch integration test | ✅ Bounded Delivered | B16 delivered: `create_propagation_signals_after_apply` test seam (`pub(crate)`) with direct-call integration tests covering disabled-by-default path (signal created, dispatch skipped) and enabled path (signal created, dispatch runs without panic using `EmptyWebhookSubscriptionResolver`); env-var toggling serialized with `std::sync::Mutex` to prevent test races |
