...
**GCP scaffold + internal GKE smoke deploy + live baseline repair + ILB applied + Phase 1/2/3/4 hardening (2026-06-18):** Core GCP infrastructure provisioned on project `ferrum-497801` (VPC, GKE, GCS, Cloud SQL Postgres). **Phase 1 internal stability** applied: node pool scaled to 2, `deletion_protection=true`, `RollingUpdate` with `maxSurge:1,maxUnavailable:0`, HPA/PDB applied. HPA metrics active: CPU `2%/70%`, memory `2%/80%`, min 1 max 3. ILB smoke: `/health` ok uptime 16568, `/ready` ready. **Phase 2 observability** deployed: Prometheus + Alertmanager on GKE with ClusterIP-only Services. Alertmanager ConfigMap generated from Secret out-of-band. Slack channel override removed; retest `GKEAlertPipelineValidationNoChannel` succeeded (POST 200, receiver `slack-and-email`, notifications email/slack both 1, failures 0). Prometheus static targets: 3 active (`alertmanager up`, `intent-api up`, `prometheus up`). **Phase 3 security hardening**: 401 response headers hardened (`Content-Type: application/json` + `Cache-Control: no-store`) committed as `9e26aaa`, image `intent-api:9e26aaa` rolled out to prod and staging. ZAP re-run: `FAIL-NEW: 0`, `WARN-NEW: 1`, `PASS: 66` (`Content-Type Header Missing` now PASS; `Non-Storable Content` x2 expected on 401 due to `Cache-Control: no-store`). Google Secret Manager API enabled; GSM secrets created for prod/staging (22 total). GKE SA granted `roles/secretmanager.secretAccessor`. ESO v2.6.0 installed, ClusterSecretStore/ExternalSecrets applied, GSM-to-K8s sync validated. Staging rotation validated. **Prod API key rotation validated (2026-06-19):** GSM `intent-rebase-prod-api-key` rotated to version [3], forced sync, hash match, Deployment restart, new pod `intent-api-7cf59448bb-6g77q`, smoke passed, `PROD_ROTATION_VALIDATED=true`. **Phase 4 bounded load test**: k6 job `k6-health-load-20260618`, 5 VUs for 5m against `http://intent-api:8080/health`. 1500/1500 checks passed, `http_req_failed=0.00%`, p95 `1.5ms`, avg `902.2µs`, throughput `4.990232/s`. Bounded health endpoint test, not full 30-minute business-path load test. **30-minute staging business-path load test with sustained-load receiver validation passed (2026-06-19):** k6 Job `business-alert-load-20260619`, 5 VUs, 8930 iterations, 8934 requests, checks 100%, 0% failure, p95 17.71ms, max 198.26ms, avg 6.97ms. Manual Alertmanager API alert `SustainedLoadReceiverValidation` posted during load (`ALERT_POST_STATUS 200`), Slack delta 1, email delta 1. Synthetic Prometheus rule firing validated during 30-minute staging business-path load (k6 Job `business-promrule-load-20260619`, 8935 requests, 0% failure, p95 17.43ms; temporary synthetic rule `StagingPipelineValidation` posted, `PROM_ALERT_STATES=firing`, Slack delta 1, email delta 1, rule cleaned up). This validates receiver delivery and synthetic rule evaluation during sustained load by manual alert and synthetic rule; it does not prove a real production SLO/SLA rule breached. **Not production-ready**: A-07 remains OPEN (self-scan prep only, no external tester engaged), public ingress/domain/TLS/Cloud Armor remains private-only by decision, broader secret rotation program not completed, A-03/A-04 re-signoff not obtained. ESO v2.6.0 installed, ClusterSecretStore/ExternalSecrets applied, GSM-to-K8s sync validated. 30-minute staging business-path load test passed. No production-ready claim.
**Production readiness:** Not production-ready. Phase 1/2/3/4 hardening applied internally (node pool 2, RollingUpdate, HPA/PDB, Prometheus/Alertmanager, 401 header hardening, GSM secrets, bounded load test, ESO sync, staging rotation validated, prod API key rotation validated, 30-min staging business-path load with sustained-load receiver validation passed, synthetic Prometheus rule firing validated, JWT dual-key support implemented, workspace verify-fast equivalent passed). Remaining blockers: A-07 WAIVED-SOLO / PRIVATE-ONLY (external pen test not approved for public claims; authenticated ZAP API scan attempt blocked by tool limitation; unauthenticated ZAP self-scan remains only ZAP evidence), public ingress/domain/TLS/Cloud Armor private-only by decision, broader secret rotation program deferred (DB URL requires Cloud SQL coord + draining; JWT dual-key support implemented but not yet rotated in production; NATS/S3 not deployed on GCP; TLS not applicable private-only), A-03/A-04 self-attested-solo — external re-signoff not obtained. **Solo/private close-out evidence:** `cargo fmt`/`check`/`clippy`/`test --lib` all passed; staging clone `a07-staging-postgres-20260618143121` deleted; final DR smoke `dr-final-smoke-20260620051418` clone+app+health/ready+authenticated API all passed (clone ready ~20m, app ready ~6s, `/health` and `/ready` ok, create+read 201/200). **System remains private-only with no public ingress. No production-ready claim is made.**
...
| Threat model v2 documented; pen test scope defined; ZAP self-scan prep completed (0 FAIL, 1 WARN accepted); A-07 remains OPEN (external pen test not engaged); no production-ready claim. |
| Load testing | 🟡 BOUNDED HEALTH + 30-MIN STAGING BUSINESS-PATH PASSED | Bounded HTTP load harness delivered (L1/L2/10min sustained); **30-minute staging business-path load test passed (8934 requests, 0% failure, p95 17.5ms)**; L3-L5 WAIVED-SOLO for non-production Phase 3; staged/production required before production claim |
...
| **P1** | External SRE sign-off | Obtain external SRE review and approval | SRE | 🟡 APPROVED WITH CONDITIONS (private-only solo operation per authorization review 2026-06-21). Historical 2026-06-15 `APPROVED WITH CONDITIONS` by DuongNguyen remains on record. Post-2026-06-15 evidence reviewed: GKE observability deployed, receivers validated under load, synthetic Prometheus rule firing validated, staging 30-min load passed, PITR clone + DR smoke validated. FIND-001 and FIND-004 accepted as non-blocking for private-only. External re-signoff required for public production. See `docs/09-operations/12-authorization-signoff-packet.md`. |
| **P1** | External security sign-off | Obtain external security review and approval | Security | 🟡 APPROVED WITH CONDITIONS (private-only solo operation per authorization review 2026-06-21). Historical 2026-06-15 `APPROVED WITH CONDITIONS` by DuongNguyen remains on record. Post-2026-06-15 evidence reviewed: RLS bounded complete (P1-S1..S5i, 13 RLC tests), 401 headers hardened + ZAP re-run confirmed, GSM+ESO provisioned, API key rotation validated. FIND-002 (RLS partial) and FIND-003 (broader rotation deferred) accepted as non-blocking for private-only. A-07 remains WAIVED-SOLO. External re-signoff required for public production. See `docs/09-operations/12-authorization-signoff-packet.md`. |
...
| **P1** | Penetration testing | Engage external pen test and remediate findings | Security | 🚫 WAIVED-SOLO / PRIVATE-ONLY — EXTERNAL NOT APPROVED (per ADR-16 + authorization review 2026-06-21). ZAP self-scan prep only (0 FAIL, 1 WARN accepted); external pen test not engaged. Engagement packet prepared at `docs/08-security/08-external-pentest-engagement.md`. Waiver valid only for private-only operation with no public ingress. Must reopen before any public/production claim. See `docs/09-operations/12-authorization-signoff-packet.md`. |
...
| **P2** | DLQ/NATS lifecycle | Bounded DLQ replay worker (`DlqReplayWorker`) delivered behind env gate (commit 274cf2a; local-dev/non-production only). G1 closed (solo self-review), G3 closed (local-dev promtool validation), G5 bounded pass; full production-grade DLQ replay worker remains Phase 4+ deferred. | Backend Lead | ✅ Bounded delivered — production-grade deferred to Phase 4+ |
