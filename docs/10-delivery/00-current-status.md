...
**GCP scaffold + internal GKE smoke deploy (2026-06-18):** Core GCP infrastructure provisioned on project `ferrum-497801` (VPC, GKE, GCS, Cloud SQL Postgres). Internal smoke deploy completed: `intent-api` container running in namespace `intent-rebase` with SQL-backed health/ready endpoints responding. Not production-ready: single-node cluster, no HPA, no ingress, no real Alertmanager receivers, no NATS, no S3, no secret manager, no PITR restore tested, no load test, no pen test.
