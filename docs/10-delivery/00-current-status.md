...
| **P2** | Webhook delivery production hardening | Outbox pattern (P2-6a schema design), background worker lifecycle (P2-6b design), HMAC signing + key rotation (P2-6c design), subscription CRUD API (P2-6d design), retry / dead-letter semantics (P2-6e design), rollback plan (P2-6f design documented in [Production Readiness Backlog](./17-production-readiness-backlog.md)), `tokio::spawn` fire-and-forget conversion | Backend Lead | 🔴 Deferred — Phase 4+ |
