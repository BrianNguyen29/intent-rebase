...
| **Design/Implementation Status** | 🟡 SLICES 1–3 DELIVERED + SLICE 4A SCHEMA ALIGNMENT + WEB-LOCAL-1 PARTIAL — outbox schema (migration 019), repository trait, in-memory implementation, bounded tests (Slice 1); `SqlxWebhookOutboxRepository` foundation added (WEB-LOCAL-1 partial); env-gated worker with claim/list-pending flow and bounded tests (Slice 2); HMAC signing foundation, dispatch boundary, and worker integration with bounded tests (Slice 3). Slice 4a schema alignment delivered: migration 020 adds `status`, `max_attempts`, `event_types` to `webhook_subscriptions` with active tenant/intent index and adds `webhook_url` to `webhook_outbox`; SQLx outbox create/read path persists and reads `webhook_url`; `WebhookSubscription` struct updated with Slice 4a fields. Subscription CRUD API routes, retry/DLQ lifecycle, and production secrets remain deferred. |
...
| **Slice 4** | Subscription CRUD API | 🔴 DEFERRED | Design documented in `docs/10-delivery/17-production-readiness-backlog.md` (P2-6d); schema aligned, routes not implemented |
...
|----|------|---------|-------|--------|
| WEB-DESIGN-1 | Resolve subscription CRUD API model before implementing routes | Schema aligned (Slice 4a): migration 020 adds `status`, `max_attempts`, `event_types` to `webhook_subscriptions` with active tenant/intent index and local-dev-safe defaults. `active_kid`/`revoked_kid` and tenant-scoped pattern matching remain deferred. Per-intent subscription model accepted for local-dev; CRUD routes not yet implemented. | Backend Lead | 🟡 PARTIAL — schema aligned, routes deferred |
