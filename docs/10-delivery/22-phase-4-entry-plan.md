...
| **Slice 5b** | DLQ table + replay UI/API | 🔴 DEFERRED | Requires DLQ table schema, replay API design, and WEB-EXT infrastructure |
...
| WEB-DESIGN-2 | Design retry/backoff/DLQ lifecycle for webhook outbox | Slice 5a delivered: bounded worker retry/backoff with retryable/terminal classification, scheduled backoff via `reschedule_retry`, and terminal failure on exhaustion/non-retryable. Full DLQ table, replay UI/API, retention policy, and runbook coverage remain deferred (Slice 5b). | Backend Lead / SRE | 🟡 PARTIAL — retry/backoff delivered; DLQ/replay/retention/runbook deferred |
