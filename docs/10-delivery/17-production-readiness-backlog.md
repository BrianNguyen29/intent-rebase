...
> **Scope:** Production readiness items and post-production work tracking; feature delivery tracked separately
...
| **Owner** | SRE |
| **Status** | 🟡 SCAFFOLD APPLIED + PHASE 1/2/3/4 HARDENING — core infrastructure provisioned on GCP project `ferrum-497801`; app running on GKE cluster with node pool 2, RollingUpdate, HPA/PDB applied, Prometheus/Alertmanager deployed, 401 headers hardened, GSM secrets provisioned, ESO v2.6.0 installed and ClusterSecretStore/ExternalSecrets applied, staging rotation validated, prod API key rotation validated, bounded load test passed, 30-minute staging business-path load test with sustained-load receiver validation passed. Remaining: ingress/TLS/domain (private-only by decision), NATS JetStream, S3, broader secret rotation program, pen test, A-03/A-04 re-signoff. |

**No overclaim:** Scaffold and internal smoke deploy with hardening applied are not production-equivalent. Phase 1 hardening (HPA, PDB, deletion protection, RollingUpdate) is applied. Phase 3 ESO v2.6.0 installed, ClusterSecretStore/ExternalSecrets applied, GSM-to-K8s sync validated, staging rotation validated, prod API key rotation validated. Broader secret rotation program (all prod secrets, JWT, grace window) not completed. Remaining open before production claim: ingress/TLS, NATS, S3, broader secret rotation, real Alertmanager Prometheus rule firing under sustained load, pen test, A-03/A-04 re-signoff. PITR restore validated against separate Cloud SQL clone (2026-06-18); Cloud SQL clone provisioning observed at >40 minutes; RPO/RTO not measured against live production traffic. Full DR program maturity remains open.

**Remaining completion items (recommended order):**
1. **Infrastructure hardening**: Node pool scaled to 2, `deletion_protection=true` applied, `RollingUpdate` with `maxSurge:1,maxUnavailable:0` applied, HPA and PDB applied on 2026-06-18. HPA metrics active: CPU `2%/70%`, memory `2%/80%`, min 1 max 3. Remaining: ingress/TLS/domain, NATS JetStream, S3, ESO auto-sync. **Ingress decision:** Private-only ClusterIP for current phase; public ingress gated (requires A-04 updated signoff, A-07 completed, domain + TLS, Cloud Armor/WAF, node pool ≥ 2, HPA + PDB, real Alertmanager, runbook).
2. **Secret manager migration**: Google Secret Manager API enabled. GSM secrets created for prod/staging (22 total: API key, DATABASE_URL, HMAC, JWT, Slack webhook, SMTP host/port/from/to/username/password). No values committed. GKE service account `production-template-gke-sa@ferrum-497801.iam.gserviceaccount.com` granted `roles/secretmanager.secretAccessor`. **ESO v2.6.0 installed from static manifest via server-side apply; release in namespace `default`. ClusterSecretStore `gcp-secret-manager` applied with GCP Secret Manager provider, project `ferrum-497801`, ADC/node SA fallback (Workload Identity pool empty). ExternalSecrets `app-secrets` applied in namespaces `intent-rebase` and `intent-rebase-staging`; both Ready=True / SecretSynced. `deletionPolicy: Retain` required (`None` rejected by CRD).** Staging rotation validated: GSM secret `intent-rebase-staging-api-key` rotated to version [2], forced sync via annotation, hash comparison matched without printing secret (`STAGING_API_KEY_ROTATION_MATCH=true`), Deployment restarted, new pod `intent-api-854f7bf87f-2wl6n`, smoke after rotation passed (`STAGING_ROTATION_SMOKE_OK`). **Prod API key rotation validated (2026-06-19):** GSM `intent-rebase-prod-api-key` rotated to version [3], forced sync, hash comparison matched without printing secret, Deployment restarted, new pod `intent-api-7cf59448bb-6g77q`, smoke passed, `PROD_ROTATION_VALIDATED=true`. **Broader secret rotation program deferred:** DB URL rotation requires coordinated Cloud SQL user/password rotation and rolling pod restart / connection draining; JWT secret rotation requires dual-key/grace-window support or simultaneous cutover (not implemented); NATS/S3 secrets are not deployed on GCP path; TLS/public ingress secrets not applicable because private-only. Validated API key rotation runbook added to `docs/09-operations/05-runbooks.md` and `docs/09-operations/08-secrets-inventory.md`. Broader secret rotation remains open.
...
6. **Load test against provisioned infra**: Bounded k6 internal load test job `k6-health-load-20260618` executed, 5 VUs for 5m against `http://intent-api:8080/health`. Result: 1500/1500 checks passed, `http_req_failed=0.00%`, p95 `http_req_duration=1.5ms`, avg `902.2µs`, throughput `4.990232/s`, thresholds passed. **30-minute staging business-path load test with sustained-load receiver validation passed** (k6 Job `business-alert-load-20260619`, 5 VUs, 8930 iterations, 8934 HTTP requests, checks `100.00% (8934/8934)`, `http_req_failed=0.00% (0/8934)`, p95 `17.71ms`, max `198.26ms`, avg `6.97ms`; Job completed and deleted after summary collection). Endpoint mix: create two intents and two versions in setup, then loop through get intent, list versions, get version, diff, rebase-preview, side-effects with tenant_id, graph nodes, health. Manual Alertmanager API alert `SustainedLoadReceiverValidation` posted during load (`ALERT_POST_STATUS 200`), Slack delta 1, email delta 1. This validates receiver delivery during sustained load by manual alert; it does not prove Prometheus rule fired under load. This is staging business-path load, not prod public ingress load.
7. **External SRE sign-off (A-03)**: Named third-party evidence against hardened infrastructure. A-03 re-signoff with named evidence after new changes has not been obtained. Assistant-generated notes require direct reviewer confirmation before any production claim.
8. **Terraform state backend**: GCS backend already configured (`backend.tf`); routine access validation and recovery docs remain.
9. **CI/CD pipeline**: Build, push, deploy automation for GKE; no CI changes have been made.
10. **Migration standardization**: `INTENT_API_RUN_MIGRATIONS=true` migration-only mode added to `intent-api` binary; K8s Job `migration-job.yaml` updated to use `intent-api:9e26aaa` image (includes `INTENT_API_RUN_MIGRATIONS=true` support + 401 header hardening). **Live Deployment rolled out** with `intent-api:9e26aaa` (pod `intent-api-79d7fd5448-bvdnv` on node `gke-production-templ-production-templ-0abeaf6e-mvlb`, IP `10.4.1.9`); health/ready endpoints responding; JWT guard passed; SQL-backed router initialized. **Live DB `_sqlx_migrations` baseline repaired and verified** (2026-06-18): on-demand backup `1781790576640` created; prep clone `sqlx-live-baseline-prep-20260618135133` created; clean DB `intent_rebase_clean` migrated; baseline imported (`COPY 22`, `sqlx_migrations_count=22`); live primary DB verified (`relation "_sqlx_migrations" already exists, skipping`, `Migrations completed successfully`); final check `public_table_count=20`, `core_tables=_sqlx_migrations,graph_edges,graph_nodes,intent_versions,webhook_outbox`, `sqlx_migrations_count=22`, `failed_migrations=0`; prep clone deleted; app health `/health` ok uptime 6664; `/ready` ready.
11. **Live DB sqlx metadata baseline/repair**: **Live primary DB baseline repaired and verified** (2026-06-18): on-demand backup `1781790576640` created on primary `production-template-postgres-ed2c5bdd`; prep clone `sqlx-live-baseline-prep-20260618135133` created, operation `DONE` with no error, RUNNABLE with private IP `10.249.0.9`; clean DB `intent_rebase_clean` created; migration-mode Job `sqlx-liveprep-clean-migrate-20260618135133` succeeded (`Migrations completed successfully`); baseline import Job `sqlx-live-baseline-import-20260618135133` succeeded (`COPY 22`, `sqlx_migrations_count=22`, `minmax=1..22`); live verify Job `sqlx-live-verify-20260618135133` succeeded against primary (`relation "_sqlx_migrations" already exists, skipping`, `Migrations completed successfully`); final check Job `sqlx-live-final-check-20260618135133` confirmed: `database=intent_rebase`, `public_table_count=20`, `core_tables=_sqlx_migrations,graph_edges,graph_nodes,intent_versions,webhook_outbox`, `sqlx_migrations_count=22`, `failed_migrations=0`; prep clone deleted; post-delete `BASELINE_PREP_CLONE_DELETED`; app health `/health` ok uptime 6664, `/ready` ready.
12. **A-07 Penetration Test execution**: Staging environment deployed and ZAP self-scan prep completed (2026-06-18). **Staging synthetic data seeded** (2026-06-19): 2 synthetic tenants, 5 intents, 5 version-2s, 2 graph nodes, 1 webhook subscription created via authenticated API calls into namespace `intent-rebase-staging`; all create requests returned `201`; markers `TENANT_COUNT 2`, `INTENT_COUNT 5`, `VERSION2_COUNT 5`, `EXTRA_SYNTHETIC_SEED_ATTEMPTED=true`. JWT token read out-of-band only; not printed or committed. **Authenticated ZAP API scan attempted** (2026-06-19): 3 attempts (canonical OpenAPI, clean OpenAPI with explicit servers, minimal generated OpenAPI) all failed with `Number of Imported URLs: 0`, `Failed to import any URL`, `ZAP_RC=3`, no JSON report. This is a tooling/import limitation, not a security pass. Unauthenticated ZAP self-scan (0 FAIL, 1 WARN accepted) remains the only ZAP evidence. **External pentest engagement packet prepared** (`docs/08-security/08-external-pentest-engagement.md`) for vendor handoff on 2026-06-19. Staging clone `a07-staging-postgres-20260618143121` created from primary, operation `DONE`, RUNNABLE, private IP `10.249.0.11`; namespace `intent-rebase-staging` created with labels `env=staging,purpose=a07-pen-test`; staging DB user password rotated to separate credential; staging K8s Secret `app-secrets` created out-of-band with staging DB URL and generated JWT/API/HMAC; staging Deployment `intent-api-6c4dd58fd4-h2mbs` and Service ClusterIP `34.118.231.110` applied; image `intent-api:9e26aaa`; staging smoke `/health` ok uptime 22, `/ready` ready. ZAP baseline self-scan (initial): `FAIL-NEW: 0`, `WARN-NEW: 2`, `PASS: 65`. ZAP re-run after 401 header hardening (image `9e26aaa`): `FAIL-NEW: 0`, `WARN-NEW: 1`, `PASS: 66` (`Content-Type Header Missing` now PASS; `Non-Storable Content` x2 expected on 401 due to `Cache-Control: no-store`). **A-07 remains OPEN** — self-scan is prep only; engagement packet prepared but no vendor selected; external tester (HackerOne/Bugcrowd/freelance) still required with evidence checklist (PDF + JSON report, HIGH/CRITICAL remediation evidence, retest confirmation).
...
| **Current State** | L1 (bounded HTTP harness with in-memory repos) and L2 (SQLx local-live with docker-compose Postgres) delivered; L4 bounded slices delivered: 6 core metrics scraped by Prometheus, 90s sustained-load smoke test passed (0% error, +4.0% RSS, FD flat), 10min extended sustained-load test passed (30,005 requests, 0% error, +4.7% RSS, FD flat), `IntentVersionCreationLowSuccessRate` alert fired via fault injection (180 errors over 6min, Alertmanager received and routed), Grafana dashboards provisioned (2 dashboards, datasource healthy, queries correct), Alertmanager receivers validated (Slack/SMTP direct transport and GKE retest passed). **30-minute staging business-path load test with sustained-load receiver validation passed** (k6 Job `business-alert-load-20260619`, 5 VUs, 8930 iterations, 8934 HTTP requests, checks 100%, 0% failure, p95 17.71ms, max 198.26ms, avg 6.97ms; endpoint mix: create intents/versions, get intent, list versions, get version, diff, rebase-preview, side-effects with tenant_id, graph nodes, health). Manual Alertmanager API alert `SustainedLoadReceiverValidation` posted during load (`ALERT_POST_STATUS 200`), Slack delta 1, email delta 1. This validates receiver delivery during sustained load by manual alert; it does not prove Prometheus rule fired under load. **This is staging business-path load, not prod public ingress load.** |
...
| **Status** | 🟡 WAIVED-SOLO (non-production Phase 3 only) — L1-L4 bounded local evidence collected; 10min sustained and one alert firing validated; **30-minute staging business-path load test with sustained-load receiver validation and synthetic Prometheus rule firing validated (not prod public ingress load)**; real SLO rule breach under sustained load remains untested; prod public ingress load remain pending; L3-L5 staged/production gated on infrastructure and external sign-off |
...
**No overclaim:** L1/L2/L4 bounded harness results are not staging or production load test results. The 30-minute staging business-path load test with sustained-load receiver validation and synthetic Prometheus rule firing is stronger evidence but is still **staging business-path load, not prod public ingress load**. Only one availability alert was triggered locally; latency, compensation, and error budget alerts were not triggered during the 30-minute test. DLQ alerts existed in rules but were not triggered because the test did not produce DLQ traffic. Real Slack/SMTP receivers were validated via direct transport and GKE retest, a manual Alertmanager API alert was posted during sustained load (Slack delta 1, email delta 1), and a temporary synthetic Prometheus rule was fired during sustained load (`PROM_ALERT_STATES=firing`, Slack delta 1, email delta 1, rule cleaned up), but a **real production SLO/SLA rule breach under sustained load was not tested.** A-03/A-04 re-signoff not obtained.
...
| **P1** | Production infra | 🟡 SCAFFOLD APPLIED + PHASE 1/2/3/4 HARDENING — GCP core scaffold applied, app running on GKE, ESO v2.6.0 installed, ClusterSecretStore/ExternalSecrets applied, staging rotation validated, 30-min staging business-path load passed | Production env verified with production hardening; deployment runbook executed |
| **P1** | Load testing (L3-L5) | 🟡 WAIVED-SOLO (non-production Phase 3 only) — L1-L4 bounded local evidence collected; **30-minute staging business-path load test passed (not prod public ingress load)**; 30min sustained alert firing + all alert types + real receivers under sustained load + prod public ingress load still required before production claim | L1-L4 bounded local evidence collected; 10min sustained + one alert firing validated; 30min sustained + all alert types + real receivers pending Phase 4; staged/production results required before production claim |
...
| Load test L3-L5 results | 🟡 WAIVED-SOLO (non-production Phase 3 only) | Production deployment; L4 10min sustained + one alert firing validated; **30-minute staging business-path load test passed (not prod public ingress load)**; 30min sustained alert firing + all alert types + real receivers under sustained load + prod public ingress load still required before production claim |
| Production infrastructure | 🟡 SCAFFOLD APPLIED + PHASE 1/2/3/4 HARDENING — ESO installed, staging rotation validated, 30-min staging business-path load passed | Staging/production deployment; prod rotation, public ingress, pen test still required |
...
| 2026-06-19 | BrianNguyen (via authorized assistant fixer) | **Evidence recorded (assistant-generated, pending direct reviewer confirmation):** ESO/GSM sync, staging rotation, 30-minute staging business-path load. A-03/A-04 re-signoff NOT obtained. A-07 remains OPEN. No production-ready claim. `No overclaim` sections updated to remove stale production-ready references. Verification: `git diff --check` pass, forbidden-claim scan clean, secret scan clean.
| 2026-06-19 | BrianNguyen (via authorized assistant fixer) | **Solo private-only waiver and self-attestation documented:** ADR-16 `docs/13-adrs/16-solo-private-operation-waiver.md` created. A-03/A-04 updated to `SELF-ATTESTED-SOLO — EXTERNAL RE-SIGNOFF NOT OBTAINED`. A-07 updated to `WAIVED-SOLO / PRIVATE-ONLY — EXTERNAL NOT APPROVED`. Historical 2026-06-15 DuongNguyen `APPROVED WITH CONDITIONS` preserved as historical record. No production-ready claim. No external signoff claimed. All docs updated consistently. Verification: `git diff --check` pass, forbidden-claim scan clean, secret scan clean. |
| 2026-06-20 | BrianNguyen (via authorized assistant fixer) | **Final solo/private close-out evidence (2026-06-20):** Verify-fast equivalent passed (`cargo fmt`/`check`/`clippy`/`test --lib` all PASS). Staging clone `a07-staging-postgres-20260618143121` deleted. Final DR smoke `dr-final-smoke-20260620051418` passed: clone ready ~20m, app ready ~6s, `/health`/`/ready` ok, authenticated API create+read 201/200. JWT dual-key support implemented (`JWT_SECRET_PREVIOUS`). All docs updated. No production-ready claim. A-07 WAIVED-SOLO. A-03/A-04 SELF-ATTESTED-SOLO. No secrets committed. Verification: `git diff --check` pass, forbidden-claim scan clean, secret scan clean. |
