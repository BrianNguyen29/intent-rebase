...
| **P1-S5h** | Compensation execute single + batch RLS tx | ✅ BOUNDED DONE (local) | Single execute RLS tx: `begin_with_tenant → executor (read-only) → record_result_with_tx + create_with_tx → commit`; non-JWT/non-RLS fallback calls `service.execute_action`; batch execute RLS tx: per-item `begin_with_tenant → executor → record_result_with_tx + create_with_tx → commit` with partial-success aggregation |
...
| **P1** | RLS transaction wrapping (P1-S1..S5 + RLC-4..12) | 🔴 IN PROGRESS | S1-S4 BOUNDED DONE (pushed); S5a..S5e BOUNDED DONE (pushed); S5f (trigger full-tx) BOUNDED DONE (local); S5g (approve/waive/reapprove + batch) BOUNDED DONE (local); S5h (execute single/batch) BOUNDED DONE (local) — per-item RLS tx with `side_effect_repo()` accessor; S5i (orchestration_runs bounded slice) BOUNDED DONE (local); forensic/artifact PENDING; RLC-4..RLC-12 BOUNDED DONE (local, 13 tests passed) |
