...
| **Owner** | Security |
| **Status** | 🟡 PENDING DIRECT REVIEWER CONFIRMATION — A-04 re-signoff with named evidence after new changes has not been obtained. Assistant-generated notes require direct reviewer confirmation before any production claim. FIND-002 (RLS partial), FIND-003 (prod rotation not done), and A-07 (external pen test not engaged) remain open. GSM+ESO sync, staging rotation, ZAP self-scan, 401 header hardening, and RLS partial address initial concerns but do not close the gate. **Not production-ready.** |
...
| **Owner** | SRE |
| **Status** | 🟡 SCAFFOLD APPLIED + PHASE 1/2/3/4 HARDENING — core infrastructure provisioned on GCP project `ferrum-497801`; app running on GKE cluster with node pool 2, RollingUpdate, HPA/PDB applied, Prometheus/Alertmanager deployed, 401 headers hardened, GSM secrets provisioned, ESO v2.6.0 installed and ClusterSecretStore/ExternalSecrets applied, staging rotation validated, bounded load test passed, 30-minute staging business-path load test passed. Remaining: ingress/TLS/domain (private-only by decision), NATS JetStream, S3, prod rotation, pen test, A-03/A-04 re-signoff. |

**No overclaim:** Scaffold and internal smoke deploy with hardening applied are not production-equivalent. Phase 1 hardening (HPA, PDB, deletion protection, RollingUpdate) is applied. Phase 3 ESO v2.6.0 installed, ClusterSecretStore/ExternalSecrets applied, GSM-to-K8s sync validated, staging rotation validated. Prod rotation was NOT performed. Remaining open before production claim: ingress/TLS, NATS, S3, prod rotation, real Alertmanager sustained-load validation, pen test, A-03/A-04 re-signoff. PITR restore validated against separate Cloud SQL clone (2026-06-18); Cloud SQL clone provisioning observed at >40 minutes; RPO/RTO not measured against live production traffic. Full DR program maturity remains open.
...
| **Status** | 🟡 WAIVED-SOLO (non-production Phase 3 only) — L1-L4 bounded local evidence collected; 10min sustained and one alert firing validated; **30-minute staging business-path load test passed (not prod public ingress load)**; remaining alert types under sustained load, real receivers under sustained load, prod public ingress load remain pending; L3-L5 staged/production gated on infrastructure and external sign-off |
...
**No overclaim:** L1/L2/L4 bounded harness results are not staging or production load test results. The 30-minute staging business-path load test is stronger evidence but is still **staging business-path load, not prod public ingress load**. Only one availability alert was triggered locally; latency, compensation, and error budget alerts were not triggered during the 30-minute test. DLQ alerts existed in rules but were not triggered because the test did not produce DLQ traffic. Real Slack/SMTP receivers were validated via direct transport and GKE retest, but sustained 30-minute alert firing under load was not executed. A-03/A-04 re-signoff not obtained.
