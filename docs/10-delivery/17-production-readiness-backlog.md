...
11. **Migration standardization**: `INTENT_API_RUN_MIGRATIONS=true` migration-only mode added to `intent-api` binary; K8s Job `migration-job.yaml` updated to use `intent-api:733e1ca` image (includes `INTENT_API_RUN_MIGRATIONS=true` support). **Live Deployment rolled out** with `intent-api:733e1ca` (pod `intent-api-6985547786-27hxd` on node `gke-production-templ-production-templ-0abeaf6e-4g41`, IP `10.4.0.16`); health/ready endpoints responding; JWT guard passed; SQL-backed router initialized. **Existing live DB `_sqlx_migrations` metadata gap remains** — the database was raw-psql migrated and the primary/live DB was NOT touched for baseline repair. The standardized Job is for future clean deploys or after live DB repair is executed.
12. **Live DB sqlx metadata baseline/repair**: Clone-then-baseline procedure **validated against a separate Cloud SQL clone** (2026-06-18): clone `sqlx-baseline-test-20260618123303` created from source `production-template-postgres-ed2c5bdd`, operation `DONE` with no error, clone RUNNABLE with private IP `10.249.0.7`. Clean DB `intent_rebase_clean` created; migration-mode Job `sqlx-clean-migrate-20260618123303` succeeded (`Migrations completed successfully`). Baseline import Job `sqlx-baseline-import-20260618123303` succeeded (`COPY 22`, `sqlx_migrations_count=22`, `minmax=1..22`). Verification Job `sqlx-clone-verify-20260618123303` succeeded against cloned existing DB (`relation "_sqlx_migrations" already exists, skipping`, `Migrations completed successfully`). Final check Job `sqlx-clone-final-check-20260618123303` confirmed: `database=intent_rebase`, `public_table_count=20`, `core_tables=_sqlx_migrations,graph_edges,graph_nodes,intent_versions,webhook_outbox`, `sqlx_migrations_count=22`, `failed_migrations=0`. Clone deleted; post-delete check `CLONE_DELETED`. **Primary/live DB was NOT touched** — `_sqlx_migrations` remains absent on the primary. Baseline repair on the live DB requires a separate execution with a maintenance window and backup.
