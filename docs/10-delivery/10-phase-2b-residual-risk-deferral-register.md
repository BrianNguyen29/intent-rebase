...
| D-11 | Trigger full-tx create+cancel RLS wrapping (P1-S5i/P3-S5) | Trigger handler-level check delivered (P1-S5e); full transaction wrapping with `begin_with_tenant → insert_request_with_tx → cancel_approved_by_intent_with_tx → commit` — P1-S5f now BOUNDED DONE (local); compensation execute full RLS tx is NO-GO bounded due to transaction-unaware executor/side_effect_repo trait boundary | P1 (Phase 3 production hardening) | Trigger approval creates a new approval request and can cancel an existing approved_by_intent approval within a full RLS-wrapped transaction. Post-commit audit is best-effort. Compensation execute full RLS tx has a documented blocker due to transaction-unaware executor/side_effect_repo trait boundary. Forensic/orchestration/artifact full RLS tx also remain pending (P1-S5i). | **Acknowledge:** Trigger handler-level check (P1-S5e) and trigger full-tx create+cancel (P1-S5f) are BOUNDED DONE (local). Full RLS transaction wrapping for compensation execute (P1-S5h) is blocked by the transaction-unaware executor/side_effect_repo trait boundary. Forensic/orchestration/artifact full RLS tx (P1-S5i) also pending. |
...
| P1 — Phase 3 Production Hardening | D-11 | Trigger full-tx create+cancel RLS wrapping (P1-S5i/P3-S5) |
