...
| D-11 | Trigger/orchestration/replay RLS wrapping (P1-S5/P3-S5) | Trigger handler-level check delivered (P1-S5e); trigger full transaction wrapping with `begin_with_tenant → insert_request_with_tx → cancel_approved_by_intent_with_tx → commit` delivered as P1-S5f bounded slice; `orchestration_runs` RLS table + `create_run_with_tx` + handler RLS path delivered as bounded orchestration slice; `replay_intent` JWT tenant guard delivered; compensation execute full RLS tx is NO-GO bounded due to transaction-unaware executor/side_effect_repo trait boundary | P1 (Phase 3 production hardening) | Trigger approval and orchestration run creation have bounded RLS-wrapped paths. Replay now has handler-level JWT tenant rejection. Compensation execute full RLS tx remains blocked by executor/side_effect_repo trait boundaries. `ingest_artifact` full RLS tx remains deferred pending a GraphService transaction boundary. Forensic bundle full SQL RLS tx is not applicable until a `SqlxBundleRepository` exists. | **Acknowledge:** Trigger handler-level check (P1-S5e), trigger full-tx create+cancel (P1-S5f), orchestration_runs RLS slice, and replay_intent guard are BOUNDED DONE (local/pushed). Full RLS transaction wrapping for compensation execute (P1-S5h) is blocked by the transaction-unaware executor/side_effect_repo trait boundary. Artifact full RLS tx and forensic SQL bundle repo remain deferred; no production-ready claim. |
