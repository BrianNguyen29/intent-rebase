...
- [ ] **Migration slice 1 (lowest-risk first).** Triage the three files called out in §9.1. Classification and execution per the §9.7 acceptance criteria (translate / pair / label). *(Partially delivered 2026-06-08: `docs/13-adrs/01-runtime-adapter.md` translated in full — see §9.13. `docs/10-delivery/04-phase-2-runtime-integrated.md` and `docs/10-delivery/01-roadmap.md` remain pending for future bounded slices.)*
