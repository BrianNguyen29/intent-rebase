...
| 4 | **P2** | Intent CLI Decoupling | local | S (1 slice) | ⬜ Not started | ❌ No |
...
- [ ] Create `crates/intent-cli/src/lib.rs` with the `Cli`, `Commands` types and a `pub fn run(cli: Cli) -> Result<...>` function.
- [ ] Convert `crates/intent-cli/src/main.rs` into a thin `bin/intent-cli.rs` that calls `intent_cli::run()`.
- [ ] Add `[[bin]]` / `[lib]` sections to `crates/intent-cli/Cargo.toml` if needed.
- [ ] Add at least one unit test for the pure command logic (e.g., a request-building helper).
- [ ] Run validation commands.

### 7.6 Acceptance Criteria

- `cargo build -p intent-cli` succeeds; running `cargo run -p intent-cli -- --help` produces the same help text as before.
