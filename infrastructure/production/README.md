...
| A-03 External SRE Sign-Off | 🟡 APPROVED WITH CONDITIONS (private-only solo operation per authorization review 2026-06-21) — EXTERNAL RE-SIGNOFF REQUIRED FOR PUBLIC PRODUCTION | Historical 2026-06-15 `APPROVED WITH CONDITIONS` by DuongNguyen remains on record. Post-2026-06-15 changes reviewed by authorized delegate (opencode AI agent) for private-only operation. FIND-001 and FIND-004 remain open. External re-signoff with named evidence has not been obtained. For production/public claims, external re-signoff is required. See `docs/09-operations/12-authorization-signoff-packet.md` and `docs/13-adrs/16-solo-private-operation-waiver.md`. |
| A-04 External Security Review | 🟡 APPROVED WITH CONDITIONS (private-only solo operation per authorization review 2026-06-21) — EXTERNAL RE-SIGNOFF REQUIRED FOR PUBLIC PRODUCTION | Historical 2026-06-15 `APPROVED WITH CONDITIONS` by DuongNguyen remains on record. Post-2026-06-15 changes reviewed by authorized delegate (opencode AI agent) for private-only operation. FIND-002, FIND-003 broader secret rotation, and A-07 remain open. External re-signoff with named evidence has not been obtained. For production/public claims, external re-signoff is required. See `docs/09-operations/12-authorization-signoff-packet.md` and `docs/13-adrs/16-solo-private-operation-waiver.md`. |
...
| Storage | GCS bucket `ire-prod-ferrum-497801-production-template-ed2c5bdd` (live metadata 2026-06-21: **no retention policy**, **no versioning**, **no public access prevention**) |
...
| 13 | **NATS JetStream internal pilot**: Internal NATS JetStream pilot provisioned on GKE (2026-06-21). Manifests: ConfigMap `nats-config` (JetStream enabled, store_dir `/data/jetstream`, max_memory 1GB, max_file 10GB, http_port 8222), PVC `nats-jetstream-pvc` (10Gi), StatefulSet `nats` (1 replica, image `nats:2.10-alpine`, probes on `/healthz` port 8222), Service `nats` (ClusterIP, ports 4222/8222, no nodePort, no public LB). Validation Job: `nats server check jetstream` OK; stream `pilot_test` created (subjects `pilot.test.>`, file storage, limits retention, 1 replica); durable consumer `pilot_consumer` created (pull mode, explicit ack, max_deliver 3); message published to `pilot.test.msg` and consumed (seq 1, acknowledged). No auth/TLS/ACL (pilot-only, internal ClusterIP). App consumer not enabled. Not production HA cluster. Per-tenant streams (ADR-15) still blocked on A-03/A-05. | Backend Lead / SRE | A-05 (infra) | 🟡 PILOT PROVISIONED + VALIDATED — NOT PRODUCTION HA |
...
## NATS JetStream Pilot Status

> **Date:** 2026-06-21
> **Status:** 🟡 PILOT — Internal single-node NATS with JetStream, not production HA cluster.
> **Namespace:** `intent-rebase`

| Resource | Kind | Status | Details |
|----------|------|--------|---------|
| `nats-config` | ConfigMap | ✅ Applied | JetStream enabled, store_dir `/data/jetstream`, max_memory 1GB, max_file 10GB, http_port 8222 (internal monitoring) |
| `nats-jetstream-pvc` | PersistentVolumeClaim | ✅ Bound | 10Gi, ReadWriteOnce, auto-provisioned |
| `nats` | StatefulSet | ✅ Ready | 1 replica, image `nats:2.10-alpine`, pod `nats-0` 1/1 Running, 0 restarts, liveness/readiness on `/healthz` port 8222 |
| `nats` | Service | ✅ ClusterIP | Ports 4222 (client) / 8222 (monitor), no nodePort, no public LB, clusterIP `34.118.235.250` |
| `nats-pilot-validation` | Job | ✅ Completed | JetStream check OK; stream `pilot_test` created (subjects `pilot.test.>`, file storage, limits retention, 1 replica, max_msgs 1000, max_bytes 1MB, max_age 1h); durable consumer `pilot_consumer` created (pull mode, explicit ack, max_deliver 3); message published to `pilot.test.msg` and consumed successfully (stream seq 1, consumer seq 1, acknowledged) |

**Caveats:**
- Single-node pilot, no clustering, no HA failover.
- No auth/TLS/ACL configured — acceptable for internal ClusterIP-only pilot but not for production multi-tenant or public-facing use.
- App consumer not enabled (`INTENT_API_NATS_CONSUMER` not set in `intent-api` Deployment).
- Per-tenant streams (ADR-15) remain design-only; Stage 2–4 blocked on A-03/A-05.
- Prometheus metrics for NATS not yet scraped (no ServiceMonitor or static target added to Prometheus config).

**Next steps for NATS maturity:**
1. Add `NATS_URL=nats://nats:4222` to `app-secrets` (via ESO or K8s Secret).
2. Enable `INTENT_API_NATS_CONSUMER=true` in `intent-api` Deployment (env-gated, safe to enable).
3. Verify app consumer connects and creates `audit_events` stream (idempotent `get_or_create_stream`).
4. Add Prometheus static target for NATS monitoring port 8222 (`/healthz`, `/jsz`, `/varz`).
5. Evaluate NATS clustering/HA for production (requires A-03 SRE signoff).
6. Add TLS/mTLS and auth (token or JWT) for production (requires A-04 security review).

---

## Forensic Storage Validation

> **Date:** 2026-06-21
> **Status:** 🟡 VALIDATED — Application-layer tamper-evidence works; storage-layer immutability NOT enforced.
> **Bucket:** `gs://ire-prod-ferrum-497801-production-template-ed2c5bdd`

### Bucket Metadata Inspection

| Property | Value | Implication |
|----------|-------|-------------|
| `retentionPolicy` | `None` | No retention period enforced; objects can be deleted or overwritten immediately |
| `versioning` | `None` | No object versioning; overwrites replace previous versions permanently |
| `publicAccessPrevention` | `None` | Bucket relies on IAM for access control (no explicit public access prevention) |
| `location` | `US-CENTRAL1` | Data residency |
| `storageClass` | `None` | Default class (likely Standard) |

### Validation Execution

| Step | Description | Result |
|------|-------------|--------|
| 1 | Generate tiny non-secret JSON test bundle (`forensic-bundle-1.json`, 171 bytes) | SHA-256 `c8da13ac...2053` |
| 2 | Upload to `forensic-validation/20260621/bundle-1.json` | ✅ Upload succeeded |
| 3 | Download and verify SHA-256 | ✅ Hash match (`c8da13ac...2053`) |
| 4 | Generate tampered bundle (`forensic-bundle-2.json`, 174 bytes, altered payload) | SHA-256 `fcec0d65...963c` |
| 5 | Upload tampered bundle to separate key; verify hash mismatch against original expected hash | ✅ Tamper detected (`fcec0d65...963c` != `c8da13ac...2053`) |
| 6 | Overwrite original `bundle-1.json` with tampered content | ✅ Overwrite succeeded (no retention policy blocking) |
| 7 | Download overwritten object and verify hash changed | ✅ Hash changed to `fcec0d65...963c` — storage-layer immutability NOT enforced |
| 8 | Delete both validation objects | ✅ Deletion succeeded (no retention policy blocking cleanup) |

### Key Findings

1. **Application-layer tamper-evidence works:** SHA-256 hash verification on download correctly detects content modification.
2. **Storage-layer immutability is NOT enforced:** The bucket has no retention policy, no versioning, and no Object Lock equivalent. Overwrite and deletion both succeed without restriction.
3. **Chain-hash (ADR-14) provides application-layer integrity:** `BundleIntegrity.previous_bundle_hash` links sequential bundles, enabling detection of missing or altered bundles in a sequence. However, this relies on the application maintaining the chain and verifying hashes — it does not prevent storage-level tampering.
4. **Not S3 Object Lock compliant:** For strict legal-hold / compliance requirements, a dedicated bucket with:
   - GCS retention policy + Bucket Lock (to prevent policy removal)
   - Object versioning (to preserve overwritten versions)
...
- This validation used tiny non-secret test data only; no production customer data was stored.
- Validation objects were deleted immediately after testing.
- The existing bucket is used for general production artifacts; a dedicated forensic bucket with stricter controls is recommended for production forensic storage.
- `FORENSIC_BUNDLE_STORAGE=s3` env gate exists in code but was not exercised against this bucket in the app runtime.

### Next Steps for Forensic Storage Maturity

1. Create a dedicated GCS bucket for forensic bundles with:
   - Retention policy (e.g., 30 days minimum)
   - Bucket Lock (to prevent retention policy removal)
...
6. Document forensic storage runbook.
...
### Load Test Results (2026-06-22 — Lane 4 Escalation)
...
### Prometheus Rule Validation (2026-06-21)

- **Temporary synthetic rule `SLOValidationSyntheticRule`** added to `prometheus-rules` ConfigMap (`expr: vector(1), for: 0s`) to validate Prometheus→Alertmanager pipeline during load.
- Rule verified firing (`state=firing`, `activeAt=2026-06-21T20:03:01.119653393Z`) throughout the 7-minute load test.
...
### Prometheus Real Metric Rule Validation (2026-06-22 — Lane 4 Escalation)
...
### App Metrics Blocker (Exact Finding)
