...
| A-07 Pen Test | 🔴 NOT APPROVED | ZAP self-scan prep only (0 FAIL, 1 WARN accepted); external pen test not engaged. No external tester engaged. |
| A-06 Load Testing (L3–L5) | 🟡 BOUNDED HEALTH + 30-MIN STAGING BUSINESS-PATH PASSED — PROD PUBLIC INGRESS LOAD OPEN | Bounded k6 health load test passed (5 VUs, 5 min, 1500/1500 checks, 0% failure, p95 1.5ms). **30-minute staging business-path load test passed** (k6 Job `business-load-20260619`, 5 VUs, 8930 iterations, 8934 HTTP requests, checks 100.00%, 0% failure, p95 17.5ms, max 45.03ms, avg 6.91ms; endpoint mix: create intents/versions in setup, then loop through get intent, list versions, get version, diff, rebase-preview, side-effects with tenant_id, graph nodes, health). Real Alertmanager receivers validated. **This is staging business-path load, not prod public ingress load.** Sustained alert firing under 30min load remains open. A-03/A-04 re-signoff not obtained. Prod public ingress load not done. |
| A-03 External SRE Sign-Off | 🟡 PENDING DIRECT REVIEWER CONFIRMATION | FIND-001 and FIND-004 remain open. A-03/A-04 re-signoff with named evidence after new changes has not been obtained. Assistant-generated review notes require direct reviewer confirmation before any production claim. |
| A-04 External Security Review | 🟡 PENDING DIRECT REVIEWER CONFIRMATION | FIND-002, FIND-003 prod rotation, and A-07 remain open. A-03/A-04 re-signoff with named evidence after new changes has not been obtained. Assistant-generated review notes require direct reviewer confirmation before any production claim. |
...
|----------------|--------------------------|
| `Production-ready` | Core GCP scaffold applied + Phase 1/2/3/4 hardening (node pool 2, RollingUpdate, HPA/PDB, Prometheus/Alertmanager, 401 headers, GSM secrets, bounded load test, ESO sync, staging rotation validated, 30-min staging business-path load passed). **Not production-ready:** A-07 open, public ingress not applied, prod rotation not done, A-03/A-04 re-signoff not obtained. |
| `FIND-001 RESOLVED` | Real Slack/SMTP credentials validated via direct transport send and temporary Alertmanager POST (2026-06-18); GKE Prometheus + Alertmanager deployed on 2026-06-18 with ClusterIP-only Services, receivers validated (retest `GKEAlertPipelineValidationNoChannel` succeeded, POST 200, notifications email/slack 1, failures 0, 3 targets active). Sustained 30min load + alert firing + A-03/A-04 reviewer re-signoff remain open. FIND-001 is PARTIALLY VALIDATED, not RESOLVED. |
| `FIND-003 RESOLVED` | GSM secrets provisioned + ESO v2.6.0 installed + ClusterSecretStore/ExternalSecrets applied + staging rotation validated. Prod rotation not done; full resolution requires prod rotation + grace window validation. Do not claim RESOLVED. |
...
- ESO v2.6.0 installed, ClusterSecretStore/ExternalSecrets applied, GSM-to-K8s sync validated, staging rotation validated. Prod rotation was NOT performed.
- 30-minute staging business-path load test passed (k6 Job `business-load-20260619`, 5 VUs, 8930 iterations, 8934 requests, 0% failure, p95 17.5ms). This is staging business-path load, not prod public ingress load. A-03/A-04 re-signoff not done.
...
| Phase 3 security hardening | ✅ Done | Google Secret Manager API enabled. GSM secrets created for prod/staging (22 total). GKE SA granted `roles/secretmanager.secretAccessor`. **ESO v2.6.0 installed, ClusterSecretStore/ExternalSecrets applied, GSM-to-K8s sync validated.** Staging rotation validated (GSM `intent-rebase-staging-api-key` rotated to version [2], forced sync, hash match, Deployment restart, smoke passed). Prod rotation was NOT performed. |
...
| Staging secret rotation validation | ✅ Done | GSM secret `intent-rebase-staging-api-key` rotated to version `[2]`. Forced ExternalSecret sync via annotation. Hash comparison matched without printing secret (`STAGING_API_KEY_ROTATION_MATCH=true`). Staging Deployment restarted; new pod `intent-api-854f7bf87f-2wl6n`. Smoke after rotation: `/health` ok, `/ready` ready, `STAGING_ROTATION_SMOKE_OK`. **Prod rotation was NOT performed.** |
| 30-minute staging business-path load test | ✅ Done | k6 Job `business-load-20260619`, 5 VUs, 30 minutes. Result: 8930 iterations, 8934 HTTP requests, checks `100.00% (8934/8934)`, `http_req_failed=0.00% (0/8934)`, p95 `17.5ms`, max `45.03ms`, avg `6.91ms`. Job completed and deleted after summary collection. Endpoint mix: create two intents and two versions in setup, then loop through get intent, list versions, get version, diff, rebase-preview, side-effects with tenant_id, graph nodes, health. **This is staging business-path load, not prod public ingress load.** |
...
- **ESO v2.6.0 installed, ClusterSecretStore and ExternalSecrets applied, GSM-to-K8s sync validated (Phase 3 complete).** Staging secret rotation validated (GSM `intent-rebase-staging-api-key` rotated to version [2], forced sync, hash match, Deployment restart, smoke passed). **Prod rotation was NOT performed.**
...
- **30-minute staging business-path load test passed (Phase 4 complete).** Bounded k6 health endpoint load test passed. This is staging business-path load, not prod public ingress load.
...
| External review gates | 🟡 PARTIAL — nuanced | FIND-001: 🟡 deployed + receivers validated (sustained-load 30min + re-signoff open); FIND-003: 🟡 GSM provisioned + ESO sync + staging rotation validated (prod rotation not done); FIND-004: 🟡 PITR clone-only validated (full DR maturity open); A-03: 🟡 PENDING DIRECT REVIEWER CONFIRMATION; A-04: 🟡 PENDING DIRECT REVIEWER CONFIRMATION; A-07: 🔴 NOT APPROVED; A-05/A-06: hardened but not production-ready. All gates require named external evidence before production claim. |
...
| 7 | **External SRE sign-off (A-03)**: Named third-party evidence against hardened infrastructure. A-03 re-signoff with named evidence after new changes has not been obtained. Assistant-generated notes require direct reviewer confirmation before any production claim. | External SRE | Items 1–6 above | 🟡 PENDING DIRECT REVIEWER CONFIRMATION |
