...
> **⚠️ CORE SCAFFOLD APPLIED + INTERNAL SMOKE DEPLOY + LIVE BASELINE REPAIRED + ILB APPLIED + PHASE 1/2/3/4 HARDENING APPLIED — not production-ready.**
>
> This directory contains an **applied GCP infrastructure scaffold**, an **internal GKE smoke deployment**, a **live Cloud SQL baseline repair**, an **applied Internal LoadBalancer**, **Phase 1 internal stability** (node pool scaled to 2, RollingUpdate, HPA/PDB applied), **Phase 2 observability** (Prometheus + Alertmanager deployed on GKE with Slack/SMTP receiver validation passed, all 3 targets up), **Phase 3 security hardening** (401 response headers hardened, ZAP re-run 1 WARN remaining `Non-Storable Content` on 401 responses which is expected due to `Cache-Control: no-store`, Google Secret Manager secrets provisioned for prod/staging), and **Phase 4 bounded load test** (k6 5 VUs / 5 min health endpoint, 1500/1500 checks passed, 0% failure) for the Intent Rebase Engine. The system is **not production-ready**: A-07 external pen test remains OPEN (self-scan prep only, no external tester engaged), public ingress/domain/TLS/Cloud Armor is not decided or applied, External Secrets Operator auto-sync from GSM to K8s is not installed, a full 30-minute business-path load test is not executed, and A-03/A-04 reviewer re-signoff with named evidence after new changes has not been obtained. All gates (FIND-001, FIND-003, FIND-004, A-05, A-06, A-07) remain OPEN or BLOCKED until named external evidence is obtained.
...
| A-06 Load Testing (L3–L5) | 🟡 BOUNDED HEALTH PASSED — FULL BUSINESS-PATH OPEN | Bounded k6 health load test passed (5 VUs, 5 min, 1500/1500 checks, 0% failure, p95 1.5ms); real Alertmanager receivers validated. Full 30min business-path load test + sustained alert firing + A-03/A-04 re-signoff not done. |
...
- Bounded k6 health load test passed (5 VUs, 5 min, 1500/1500 checks, 0% failure); full 30min business-path load test + A-03/A-04 re-signoff not done.
...
| Phase 3 security hardening | ✅ Done | Google Secret Manager API enabled. GSM secrets created for prod/staging (22 total). GKE SA granted `roles/secretmanager.secretAccessor`. K8s runtime still uses K8s Secret; GSM is canonical store step 1. ESO auto-sync not installed. |
...
- Google Secret Manager secrets created for prod/staging (22 total) but K8s runtime still uses K8s Secret. ESO/CSI auto-sync not installed (Phase 3 partial).
...
- Bounded k6 health endpoint load test passed (Phase 4 bounded). Full 30-minute business-path load test not executed.
- A-03/A-04 reviewer re-signoff with named evidence after new changes has not been obtained.
...
| External review gates | 🟡 PARTIAL — nuanced | FIND-001: 🟡 deployed + receivers validated (sustained-load 30min + re-signoff open); FIND-003: 🟡 GSM provisioned (ESO auto-sync + rotation open); FIND-004: 🟡 PITR clone-only validated (full DR maturity open); A-05/A-06: hardened but not production-ready (A-07 open, public ingress not applied, full 30-min business-path load test not done, A-03/A-04 re-signoff not obtained). All gates require named external evidence before production claim. |
...
| 6 | **Load test against provisioned infra**: Bounded k6 internal load test job `k6-health-load-20260618` executed, 5 VUs for 5m against `http://intent-api:8080/health`. Result: 1500/1500 checks passed, `http_req_failed=0.00%`, p95 `http_req_duration=1.5ms`, avg `902.2µs`, throughput `4.990232/s`, thresholds passed. This is bounded health endpoint load, not full 30-minute business-path/load test. | Backend Lead / SRE | A-05, A-03 | 🟡 BOUNDED HEALTH LOAD TEST PASSED — FULL BUSINESS-PATH LOAD TEST OPEN |
