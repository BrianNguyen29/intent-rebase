...
| FIND-001 Production telemetry / Alertmanager real receivers | 🟡 PARTIALLY VALIDATED — RECEIVERS OK / GKE STACK OPEN | Alertmanager ConfigMap + `alertmanager-prod.yml` placeholders exist; real Slack/SMTP credentials supplied out-of-band to K8s Secrets `app-secrets` in both `intent-rebase` and `intent-rebase-staging` namespaces (18 keys each, including both uppercase and lowercase Slack/SMTP keys). Direct transport validation passed: Slack webhook returned `200 ok`; SMTP send returned `sent` with `0` refused. Temporary Alertmanager container validation passed: health OK, POST to `/api/v2/alerts` returned `200`, alert `SlackSMTPValidationTest` showed state `active` with receiver `slack-and-email`, logs showed no relevant errors. GKE Alertmanager Deployment and Prometheus rules are NOT deployed; production telemetry stack remains open. |
...
| `FIND-001 RESOLVED` | Real Slack/SMTP credentials validated via direct transport send and temporary Alertmanager POST (2026-06-18), but GKE Alertmanager/Prometheus Deployment is not deployed; production telemetry stack remains open. FIND-001 is PARTIALLY VALIDATED, not RESOLVED. |
...
| Alert validation (Slack/SMTP) | ✅ Done | Real Slack/SMTP credentials supplied out-of-band to K8s Secrets `app-secrets` in both `intent-rebase` and `intent-rebase-staging` namespaces (18 keys each, including both uppercase and lowercase Slack/SMTP keys). Direct transport validation: Slack webhook returned `200 ok`; SMTP send returned `sent` with `0` refused. Temporary Alertmanager container validation: health OK, POST to `/api/v2/alerts` returned `200`, alert `SlackSMTPValidationTest` state `active`, receiver `slack-and-email`, logs showed no relevant errors. Temp files cleaned up. **GKE Alertmanager/Prometheus Deployment is NOT deployed; this is credential/delivery validation only, not production telemetry connected.** |
...
- Real Slack/SMTP credentials were validated via direct transport send and temporary Alertmanager container (2026-06-18), but GKE Alertmanager Deployment and Prometheus rules are NOT deployed. Production telemetry stack remains open.
...
| 4 | **Monitoring + Alertmanager**: Real Slack/SMTP credentials supplied out-of-band to K8s Secrets `app-secrets` in both `intent-rebase` and `intent-rebase-staging` namespaces (18 keys each, including both uppercase and lowercase Slack/SMTP keys). Direct transport validation passed: Slack webhook returned `200 ok`; SMTP send returned `sent` with `0` refused. Temporary Alertmanager container validation passed: health OK, POST to `/api/v2/alerts` returned `200`, alert `SlackSMTPValidationTest` showed state `active` with receiver `slack-and-email`, logs showed no relevant errors. Temp files cleaned up. **GKE Alertmanager Deployment and Prometheus rules are NOT deployed; production telemetry stack remains open.** | SRE / Backend Lead | A-05, A-06 | 🟡 RECEIVERS VALIDATED — GKE STACK OPEN |
